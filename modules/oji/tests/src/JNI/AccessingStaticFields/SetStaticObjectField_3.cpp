...
JNI_OJIAPITest(JNIEnv_SetStaticObjectField_3)
{
  GET_JNI_FOR_TEST
  ...
  env->SetStaticObjectField(clazz, (jfieldID)100, str);
  ...
  if((str_chars != NULL) && ((int)len == 3)){
     ...
     return TestResult::PASS("SetStaticObjectField(fieldID incorrect positive) return correct value");
  }else{
     return TestResult::FAIL("SetStaticObjectField(fieldID incorrect positive) return incorrect value!!!!");
  }
