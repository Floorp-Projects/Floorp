...
JNI_OJIAPITest(JNIEnv_GetStaticObjectField_5)
{
  GET_JNI_FOR_TEST
  ...
  if((str_chars != NULL) && ((int)len == 4)){
     printf("Value is \"asdf\"!!\n\nIt's correct!\n");
     return TestResult::PASS("GetStaticObjectField(all right) return correct value");
  }else{
     return TestResult::FAIL("GetStaticObjectField(all right) return incorrect value!!!!");
  }
