...
JNI_OJIAPITest(JNIEnv_ReleaseCharArrayElements_3)
{
  GET_JNI_FOR_TEST
  ...
  env->ReleaseCharArrayElements(arr, val, JNI_COMMIT);
  ...
  if((valu[0]=='q') && (valu[1]=='w') && (valu[2]=='e')){
     return TestResult::PASS("ReleaseCharArrayElements(arr, val, JNI_COMMIT) is correct");
  }else{
     return TestResult::FAIL("ReleaseCharArrayElements(arr, val, JNI_COMMIT) is incorrect");
  }
}