...
JNI_OJIAPITest(JNIEnv_GetObjectArrayElement_4)
{
  GET_JNI_FOR_TEST
  ...
  jstring str_n = (jstring)env->GetObjectArrayElement(arr, (jsize) -1);
  jthrowable excep  = env->ExceptionOccurred();
  if(env->IsInstanceOf(excep, clazz_exp)){
    printf("ArrayIndexOutOfBoundsException is thrown. It is correct!\n");
     return TestResult::PASS("GetObjectArrayElement(index < 0 ) returns correct value - empty array");
  }else{
     return TestResult::FAIL("GetObjectArrayElement(index < 0 ) returns incorrect value");
  }
