...
JNI_OJIAPITest(JNIEnv_ReleaseFloatArrayElements_1)
{
  GET_JNI_FOR_TEST
  ...
  env->ReleaseFloatArrayElements(arr, val, 0);
  ...
  if((valu[0]==0) && (valu[1]==1) && (valu[2]==2)){
     return TestResult::PASS("ReleaseFloatArrayElements(arr, val, 0) is correct");
  }else{
     return TestResult::FAIL("ReleaseFloatArrayElements(arr, val, 0) is incorrect");
  }
}