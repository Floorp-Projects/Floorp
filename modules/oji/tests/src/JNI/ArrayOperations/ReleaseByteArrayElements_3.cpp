...
JNI_OJIAPITest(JNIEnv_ReleaseByteArrayElements_3)
{
  GET_JNI_FOR_TEST
  ...
  env->ReleaseByteArrayElements(arr, val, JNI_COMMIT);
  ...
  if((valu[0]==0) && (valu[1]==1) && (valu[2]==2) && (val[0]==0)){
     return TestResult::PASS("ReleaseByteArrayElements(arr, val, JNI_COMMIT) is correct");
  }else{
     return TestResult::FAIL("ReleaseByteArrayElements(arr, val, JNI_COMMIT) is incorrect");
  }
}