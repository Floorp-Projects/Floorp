...
JNI_OJIAPITest(JNIEnv_ReleaseLongArrayElements_1)
{
  GET_JNI_FOR_TEST
  ...
  env->ReleaseLongArrayElements(arr, val, 0);
  ...
  if((valu[0]==0) && (valu[1]==1) && (valu[2]==2)){
     return TestResult::PASS("ReleaseLongArrayElements(arr, val, 0) is correct");
  }else{
     return TestResult::FAIL("ReleaseLongArrayElements(arr, val, 0) is incorrect");
  }
}