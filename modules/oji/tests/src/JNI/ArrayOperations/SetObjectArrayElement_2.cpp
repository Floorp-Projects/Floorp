...
JNI_OJIAPITest(JNIEnv_SetObjectArrayElement_2)
{
  GET_JNI_FOR_TEST
  ...
  env->SetObjectArrayElement(arr, (jsize)0, NULL);

  jstring str_n = (jstring)env->GetObjectArrayElement(arr, (jsize) 0);
  if(str_n == NULL){
     return TestResult::PASS("SetObjectArrayElement(all correct, element = NULL) returns correct value - empty array");
  }else{
     return TestResult::FAIL("SetObjectArrayElement(all correct, element = NULL) returns incorrect value");
  }
