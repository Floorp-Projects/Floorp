...
TEST_F(SynchronousConnectionInterruptionTest,
  ...
  ASSERT_EQ(NS_OK, mConnection->CreateStatement(sql, getter_AddRefs(stmt)));
...
TEST_F(SynchronousConnectionInterruptionTest,
       interruptOnTheSameThreadWillFail) {
#ifdef DEBUG
  ASSERT_DEATH(mConnection->Interrupt(),
               "threadOpenedOn != NS_GetCurrentThread");
#else
  SUCCEED();  // The test is relevant only for debug builds
#endif
}

TEST(SynchronousConnectionNonInterruptible, cannotBeInterrupted)
{
#ifdef DEBUG
  nsCOMPtr<mozIStorageConnection> connection = getDatabase(nullptr);
  ASSERT_DEATH(connection->Interrupt(), "Interrupt method not allowed");
#else
  SUCCEED();  // The test is relevant only for debug builds
#endif
}
