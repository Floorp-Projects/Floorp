...
public:
  ...
  // aFileExtension --> the extension we need to append to our temp file
  // INCLUDING the ".". i.e. .mp3
  nsExternalAppHandler * CreateNewExternalHandler(nsIMIMEInfo * aMIMEInfo,
                                                  const char * aFileExtension,
                                                  nsISupports * aWindowContext);
 
  // GetMIMEInfoForMimeTypeFromDS --> Given a content type, look up the user
  // override information to see if we have a mime info object representing
  // this content type. The user over ride information is contained
  // in a in memory data source....
  nsresult GetMIMEInfoForMimeTypeFromDS(const char * aContentType,
                                        nsIMIMEInfo ** aMIMEInfo);
  
  // GetMIMEInfoForExtensionFromDS --> Given an extension, look up the user
  // override information to see if we have a mime info object representing
  // this extension. The user over ride information is contained
  // in a in memory data source....
  ...
  // platformAppPath --> a platform specific path to an application that we
...
protected:
  ...
  virtual nsresult GetMIMEInfoForMimeTypeFromExtras(const char * aContentType,
                                                    nsIMIMEInfo ** aMIMEInfo);
  virtual nsresult GetMIMEInfoForExtensionFromExtras(const char * aContentType,
                                                     nsIMIMEInfo ** aMIMEInfo);
