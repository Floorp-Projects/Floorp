...
let PromptTestUtils = {
  ...
  handlePrompt(
    ...
    let win;
    if (dialog.args.modalType === Services.prompt.MODAL_TYPE_WINDOW) {
      win = dialog.ui.prompt?.opener;
      if (!win) {
        // Standalone prompt window, wait for window close instead
        promptClosePromise = BrowserTestUtils.windowClosed(dialog.ui.prompt);
      }
    } else {
      // Tab prompts always have a parent window
   ...
   */
  waitForPrompt(parent, { modalType, promptType = null } = {}) {
    return new Promise((resolve, reject) => {
      if (!modalType) {
        return reject(new Error("modalType is mandatory"));
      }
      ...
      let isTabPrompt = modalType !== Services.prompt.MODAL_TYPE_WINDOW;

      let topic = isTabPrompt
      ...
      Services.obs.addObserver(function observer(subject) {
        ...
        if (isTabPrompt && dialog.args.modalType !== modalType) {
          return;
        }
