...
const COVERAGE_ENABLED_PREF = "toolkit.telemetry.coverage.enabled";
const LOG_LEVEL_PREF = "toolkit.telemetry.coverage.log-level";
const OPT_OUT_PREF = "toolkit.telemetry.coverage.opt-out";
const ALREADY_RUN_PREF = `toolkit.telemetry.coverage.already-run.v${COVERAGE_VERSION}`;
const COVERAGE_UUID_PREF = `toolkit.telemetry.coverage.uuid.v${COVERAGE_VERSION}`;
const TELEMETRY_ENABLED_PREF = "datareporting.healthreport.uploadEnabled";
const REPORTING_ENDPOINT_BASE_PREF = `toolkit.telemetry.coverage.endpoint.base`;
...
var CoveragePing = Object.freeze({
  async startup() {
    ...
    if (!Services.prefs.getBoolPref(COVERAGE_ENABLED_PREF)) {
