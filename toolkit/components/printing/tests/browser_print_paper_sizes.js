...
async function selectPaperOptionWithValue(helper, value) {
  ...
  info(
    "selectPaperOptionWithValue: Current value: " +
      paperSelect.value +
      ", new value: " +
      value
  );

  let targetOptionIndex = Array.from(paperSelect.options).findIndex(
    o => o.value == value
  );
  ok(targetOptionIndex > -1, "Paper size option exists for value:" + value);

  await helper.openMoreSettings({ scroll: true });
  paperSelect.focus();
  EventUtils.synthesizeKey("VK_SPACE", {}, helper.win);
  await helper.awaitAnimationFrame();

  while (paperSelect.selectedIndex !== targetOptionIndex) {
    let directionKey =
      targetOptionIndex > paperSelect.selectedIndex
        ? "KEY_ArrowDown"
        : "KEY_ArrowUp";
    EventUtils.synthesizeKey(directionKey, {}, helper.win);
  }
  EventUtils.synthesizeKey("KEY_Enter", {}, helper.win);
  is(paperSelect.value, value, "New option was selected");
  await helper.awaitAnimationFrame();
}
