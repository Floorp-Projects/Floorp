...
add_task(
  ...
  async function test_eventpage_runtime_parentApiCall_resets_timeout() {
    ...
    let extension = ExtensionTestUtils.loadExtension({
      ...
      background() {
        ...
        browser.runtime.onSuspend.addListener(() => {
          browser.test.sendMessage("done", count);
        });
      },
    });
    ...
    let count = await extension.awaitMessage("done");
    equal(count, 30, "Parent API calls did prevent background suspension.");

    let lifetime = Date.now() - start;
    ok(lifetime > 300, `Background script suspended after ${lifetime}ms.`);
