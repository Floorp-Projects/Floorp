...
add_task(async function test_extension_page_tabs_create_reload_and_close() {
  ...
  let contentPage = await ExtensionTestUtils.loadContentPage(tabURL, {extension});
  ...
  equal(contextEvents[0].url, extensionPageURL,
  ...
  equal(extensionPageURL, extensionPageURL2,
  ...
  equal(contextEvents[1].eventType, "load", "create new ExtensionContext for tab");
  equal(contextEvents[1].url, extensionPageURL2,
  ...
  equal(contextEvents[0].url, extensionPageURL2,
