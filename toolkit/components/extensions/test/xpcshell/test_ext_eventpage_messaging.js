...
add_setup(() => {
  ...
  Services.prefs.setIntPref("extensions.background.idle.timeout", 5000);
});
...
add_task(async function test_runtime_onMessage_cancels_suspend() {
  await loadEventPageAndExtensionPage({
    ...
    extensionPageScript() {
      ...
      // Note: if test hangs here, it could mean that the background has already
      // suspended. Increase the timeout in the setup to fix that.
...
add_task(async function test_runtime_Port_onMessage_cancels_suspend() {
  await loadEventPageAndExtensionPage({
    ...
    extensionPageScript() {
      ...
      // Note: if test hangs here, it could mean that the background has already
      // suspended AND that runtime.connect() does unexpectedly not wake up the
      // already-suspended background page (bug 1851873).
      browser.test.log("Waiting for background to be suspended");
    },
  });
});
