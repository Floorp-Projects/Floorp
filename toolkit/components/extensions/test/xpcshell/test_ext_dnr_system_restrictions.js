...
add_task(async function non_system_request_with_disallowed_scheme() {
  ...
  await contentPage.spawn(null, async () => {
    const { document } = content;
    // To make sure that the test does not pass trivially, we verify that it is
    // not the system principal (because dnr_ignores_system_requests already
    // tests that) and not a null principal (because that translates to a void
    // "initiator" in the DNR API, which would pass access checks).
    Assert.ok(
