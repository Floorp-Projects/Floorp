...
class HeaderVisitor final : public nsIHttpHeaderVisitor {
 public:
  ...
  HeaderVisitor(google::protobuf::Map<std::string, std::string>* aHeaders)
      : mHeaders(aHeaders) {}
...
nsresult ViaductRequest::LaunchRequest(
  ...
  for (auto& [k, v] : headers) {
    auto name = nsDependentCString(k.c_str(), k.size());
    auto value = nsDependentCString(v.c_str(), v.size());
    rv = httpChannel->SetRequestHeader(name, value, false /* merge */);
    NS_ENSURE_SUCCESS(rv, rv);
  }
