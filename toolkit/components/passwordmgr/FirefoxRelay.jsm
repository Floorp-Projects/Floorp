...
const config = (function() {
  const baseUrl = Services.prefs.getStringPref("signon.firefoxRelay.base_url");
  return {
...
class RelayOffered {
  ...
  async *autocompleteItemsAsync(_origin, scenarioName) {
    if (isSignup(scenarioName)) {
...
class RelayEnabled {
  async *autocompleteItemsAsync(origin, scenarioName) {
    if (isSignup(scenarioName) && (await hasFirefoxAccountAsync())) {
      const [title, subtitle] = await formatMessages(
