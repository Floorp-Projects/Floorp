...
class GraphImpl {
  ...
  drawSparseValues = (dataSet, title, config) => {
    ...
    rollingAverage.slice(-1).forEach(({ value }) => {
      ...
      ctx.font = "9px Arial";
      ctx.textAlign = "left";
      ctx.fillText(fixed(value), 5, this.height - 5);
    });
    ...
    if (title) {
      ...
      ctx.font = "10px Arial";
      ctx.textAlign = "left";
      ctx.fillText(`${title}${config.toRate ? "/s" : ""}`, 5, 10);
    }
    ctx.font = "9px Arial";
    ctx.fillStyle = this.maxColor(range.max);
    ctx.textAlign = "right";
    ctx.fillText(fixed(range.max), this.width - 5, 7);
    ctx.fillStyle = this.minColor(range.min);
    ctx.fillText(fixed(range.min), this.width - 5, this.height - 5);
  };
}
