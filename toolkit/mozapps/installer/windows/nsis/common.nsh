...
; According to the Processes docs the file extension should not be included but
; for some reason it only works properly when it is included
!macro CLOSE_APP PROC_NAME MSG
loop:
  Processes::FindProcess ${PROC_NAME}
  Pop $R0
  IntCmp $R0 0 done
  MessageBox MB_OKCANCEL|MB_ICONSTOP ${MSG} IDCANCEL exit 0
  Processes::KillProcess ${PROC_NAME}
  Sleep 2000
  Goto loop
exit:
  Quit
