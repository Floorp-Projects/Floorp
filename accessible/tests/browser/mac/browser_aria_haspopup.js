...
function stateChange(aState, aIsExtraState, aIsEnabled, aTarget) {
  return [
    ...
    evt => {
      ...
      return (
        evt.state == aState &&
        evt.isExtraState == aIsExtraState &&
        aIsEnabled == evt.isEnabled &&
        getAccessible(aTarget) == evt.accessible
      );
    },
  ];
}
...
/**
 * Change or remove ARIA attribute.
 */
...
addAccessibleTask(
  `
  <button aria-haspopup="false" id="false1">action</button>
