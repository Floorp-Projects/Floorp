...
package PLIF::Service::ContactMethod::AIM;
use strict;
use vars qw(@ISA);
use PLIF::Output::Generic::AIM;
@ISA = qw(PLIF::Output::Generic::AIM);
1;

# XXX This inherits from PLIF::Output::Generic::AIM, which is silly
# (or is it?). Should it instead be a pure service, but install the
# AIM output service? Should this just be a part of the output
...
sub register {
    ...
    $fieldFactory->registerField($app, 'contact', 'aim', 'string');
}
