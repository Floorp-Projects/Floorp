...
$sql = "SELECT TM.ID, TM.Name, TM.DateAdded, TM.DateUpdated, TM.Homepage, TM.Description, TM.Rating, TM.TotalDownloads, TM.downloadcount, TV.vID, TV.Version, TV.MinAppVer, TV.MaxAppVer, TV.Size, TV.DateAdded AS VerDateAdded, TV.DateUpdated AS VerDateUpdated, TV.URI, TV.Notes, TA.AppName, TOS.OSName
    ...
    if ($category && $category !=="%") {
        $sql .="INNER  JOIN t_categoryxref TCX ON TM.ID = TCX.ID
        INNER JOIN t_categories TC ON TCX.CategoryID = TC.CategoryID ";
    }
    if ($editorpick=="true") {
        $sql .="INNER JOIN t_reviews TR ON TM.ID = TR.ID ";
    }
...
<?php
    ...
    echo"<SPAN style=\"itemdescription\">Released on $verdateadded</SPAN><BR>
    ...
    if ($homepage) {
        echo"<DIV class=\"iconbar\"><A HREF=\"$homepage\"><IMG SRC=\"/images/home.png\" BORDER=0 HEIGHT=34 WIDTH=34 TITLE=\"$name Homepage\" ALT=\"\">Homepage</A></DIV>";
    }
    ...
    if ($homepage) {
        echo"<SPAN style=\"font-size:10pt\">Having a problem with this Extension? For Help and Technical Support, visit the <A HREF=\"$homepage\">Extension's Homepage</A>.</SPAN>";
    }

    echo"<UL style=\"font-size:10pt\">";

    if ($homepage) {
        echo"<LI> <A HREF=\"$homepage\">Extension Home Page</A>";
    }
    if ($appname !="Thunderbird") {
        echo"<LI> <a href=\"install.php/$filename?id=$id&vid=$vid\" TITLE=\"Right-click to Save\">Download Extension</A>";
    }
...
if ($num_pages>1) {
    ...
    $sql_result = mysql_query($sql, $connection) or trigger_error("MySQL Error ".mysql_errno().": ".mysql_error()."", E_USER_NOTICE);
        ...
        while ($row = mysql_fetch_array($sql_result)) {
            ...
            echo"$title<br>\n";
            if ($pick=="YES") {
                echo"<SPAN class=\"itemdescription\">&nbsp;&nbsp;&nbsp;$date Editors Pick<BR>\n";
            }
            echo"<BR>\n";
            echo"$body</SPAN><BR>\n";
        ...
        if ($num_results=="0") {
            ...
            To see what other users think of this extension, view the <A HREF=\"moreinfo.php?".uriparams()."&id=$id&page=comments\">User Comments...</A>
            ";
        }
        echo"</DIV>\n";
