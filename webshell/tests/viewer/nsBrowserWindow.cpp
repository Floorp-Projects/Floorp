...
nsBrowserWindow::DoFileOpen()
{
  ...
  if (GetFileFromFileSelector(parentWindow, getter_AddRefs(file),
    ...
    if (uri) {
      ...
      nsCOMPtr<nsIWebNavigation> webNav(do_QueryInterface(mWebBrowser));
      webNav->LoadURI(NS_ConvertUTF8toUTF16(spec).get(),
...
static void DumpAWebShell(nsIDocShellTreeItem* aShellItem, FILE* out, PRInt32 aIndent)
{
  ...
  str.Assign(name);
  fputs(NS_LossyConvertUTF16toASCII(str).get(), out);
  ...
  case VIEWER_SELECT_STYLE_LIST:
    {
      ...
      if (nsnull != shell) {
        ...
        for (index = 0; index < count; index++) {
          ...
          fputs(NS_LossyConvertUTF16toASCII(title).get(), stdout);
          fprintf(stdout, "\" %s%s\n", 
  ...
  case VIEWER_SELECT_STYLE_DEFAULT:
    {
      ...
      if (nsnull != shell) {
        ...
        if (doc) {
          ...
          fputs(NS_LossyConvertUTF16toASCII(defaultStyle).get(), stdout);
          fputs("\"\n", stdout);
