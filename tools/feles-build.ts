...
function printHelp(): void {
  ...
  console.log("  init       Initialize the development environment");
  console.log("  patch      Run patcher actions (--action)");
  console.log("");
...
async function main(): Promise<void> {
  ...
  switch (command) {
    ...
    case "init": {
      if (argv.includes("--help") || argv.includes("-h")) {
        console.log("Usage: feles-build init");
        return;
      }
      await runInit();
      break;
    ...
    case "patch": {
      if (argv.includes("--help") || argv.includes("-h")) {
        console.log("Usage: feles-build patch --action <apply|create|init>");
        return;
      }
