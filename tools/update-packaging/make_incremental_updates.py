...
def create_partial_patch_for_file(
    ...
    if not (from_marfile_entry.sha(), to_marfile_entry.sha()) in shas:
        ...
        copy_file(
            src_file_abs_path, os.path.join(patch_info.work_dir, file_in_manifest_name)
        )
        patch_info.archive_files.append('"' + file_in_manifest_name + '"')
...
def create_add_patch_for_file(to_marfile_entry, patch_info):
    ...
    copy_file(
        to_marfile_entry.abs_path,
        os.path.join(patch_info.work_dir, to_marfile_entry.name),
    )
...
def create_partial_patches(patches):
    ...
    try:
        ...
        for patch in patches:
            ...
            print(
                "done with patch %s/%s time (%.2fs/%.2fs/%.2fs) (mar/patch/total)"
                % (
                    str(patch_num),  # NOQA: E501
                    str(len(patches)),
                    mar_extract_time - startTime,
                    time.time() - mar_extract_time,
                    time.time() - startTime,
                )
            )  # NOQA: E501
            patch_num += 1
