...
export async function runChildBuild(isDev: boolean): Promise<void> {
  const { readBuildid2 } = await import("../misc/update/buildid2.ts");
  const buildid2 = (await readBuildid2(resolveFromRoot("_dist"))) ||
    "default-build-id";
  const mode = isDev ? "dev" : "prod";
  const childBuildPath = resolveFromRoot("tools/misc/launchDev/child-build.ts");
...
export async function runChildDev(
  ...
  const buildid2 = (await readBuildid2(resolveFromRoot("_dist"))) ||
    "default-build-id";
  const childDevPath = resolveFromRoot("tools/misc/launchDev/child-dev.ts");
