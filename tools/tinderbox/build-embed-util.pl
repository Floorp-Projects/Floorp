...
sub BuildIt {
	...
	my $embed_dir = "$build_dir/$Settings::Topsrcdir/${Settings::ObjDir}/dist/Embed";
    my $full_embed_binary_name = "$embed_dir/gtkEmbed";
    my $exit_early = 0;
    my $start_time = 0;
    ...
    while (not $exit_early) {
        ...
        if ($build_status ne 'busted' and BinaryExists($full_binary_name)) {
		  print_log "$binary_basename binary exists, building now.\n";
          ...
          my $statusEmbed = run_shell_command "gmake";
            if ($statusEmbed != 0) {
              $build_status = 'busted';
        ...
        # Gecko tests, if any, go here.
        if ($build_status ne 'busted' and BinaryExists($full_embed_binary_name)) {
