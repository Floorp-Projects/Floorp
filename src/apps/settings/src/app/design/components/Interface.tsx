        ...
        <Card>
            ...
            <CardContent>
                <div className="space-y-6">
                    <div>
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-3">
                            {interfaceOptions.map((option) => (
                                <label
                                    key={option.value}
                                    className={`flex flex-col items-center border rounded p-2 cursor-pointer relative hover:bg-accent ${getValues("design") === option.value
                                        ? 'bg-accent text-accent-foreground dark:bg-accent dark:text-accent-foreground'
                                        : 'bg-transparent'
                                        }`}
                                >
                                    <img src={option.image} alt={option.title} className="w-40 h-24 mb-2 object-contain" />
                    ...
                    <div className="mt-6">
                        <h3 className="text-base font-medium mb-2">{t("design.otherInterfaceSettings")}</h3>
                        <div className="flex items-center justify-between">
                            <div className="space-y-1">
                                <label>{t("design.useFaviconColorToBackgroundOfNavigationBar")}</label>
                            </div>
                            <input
                                type="checkbox"
