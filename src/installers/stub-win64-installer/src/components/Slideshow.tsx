    ...
    return (
        <div className="flex flex-1 w-full card bg-base-200 shadow-md relative">
            <div className="fixed top-4 left-4 flex items-center space-x-2 gap-2 z-20">
                ...
                <h3 className="text-lg font-semibold">{t('app.installer.title')}</h3>
            </div>

            <div className="card-body p-6 flex flex-col h-full">
                <div className="flex flex-col md:flex-row flex-1 gap-8 items-center justify-center">
                    <div className="flex-1 flex justify-center items-center">
                        <div className="w-44 h-44 md:w-64 md:h-64 text-primary flex justify-center items-center">
            ...
            <div className="absolute bottom-6 left-0 right-0 flex justify-between items-center px-6">
                <button className="btn btn-circle" onClick={onPrevSlide} aria-label={t('slides.navigation.previous', '前へ')}>
                ...
                <div className="flex gap-2 justify-center">
                    {slides.map((_, index) => (
                        <button
                            key={index}
                            className={`btn btn-sm ${index === currentSlide ? 'btn-primary' : 'btn-ghost'}`}
                            onClick={() => onGoToSlide(index)}
                            aria-label={t('slides.navigation.goto', 'スライド{{number}}へ', { number: index + 1 })}
                        >
                            {index + 1}
                        </button>
                    ))}
                ...
                <button className="btn btn-circle" onClick={onNextSlide} aria-label={t('slides.navigation.next', '次へ')}>
                    <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
