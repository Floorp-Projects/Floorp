...
Solid-XUL パッケージ（`src/packages/solid-xul/`）は、SolidJS のリアクティブコンポーネントを Firefox の XUL（XML User Interface Language）システムとシームレスに連携させるための重要なブリッジです。この統合により、Floorp は Firefox の既存インターフェースフレームワークとの完全な互換性を維持しつつ、モダンでリアクティブな UI 体験を提供できます。

## アーキテクチャ

### コアコンセプト

SolidJS-XUL 統合は、2 つの異なる UI パラダイム間に双方向のブリッジを作成することで機能します：

- **SolidJS**: 細粒度リアクティビティを持つモダンなリアクティブフレームワーク
- **XUL**: Firefox で使われる Mozilla の XML ベース UI システム

```
┌─────────────────────────────────────────────────────────────┐
│                  Solid-XUL アーキテクチャ                    │
├─────────────────────────────────────────────────────────────┤
│  SolidJSコンポーネント（TypeScript/JSX）                    │
│  ┌─────────────┐ ┌─────────────┐ ┌─────────────────────┐   │
│  │ リアクティブ │ │ カスタム    │ │ イベント処理        │   │
│  │ コンポーネント│ │ フック      │ │ & 状態管理          │   │
│  └─────────────┘ └─────────────┘ └─────────────────────┘   │
├─────────────────────────────────────────────────────────────┤
│  ブリッジ層（solid-xulパッケージ）                          │
│  ┌─────────────┐ ┌─────────────┐ ┌─────────────────────┐   │
│  │ JSX         │ │ XUL         │ │ イベントブリッジ      │   │
│  │ ランタイム   │ │ 要素        │ │ & APIラッパー         │   │
│  └─────────────┘ └─────────────┘ └─────────────────────┘   │
├─────────────────────────────────────────────────────────────┤
│  XULシステム（Firefoxネイティブ）                          │
│  ┌─────────────┐ ┌─────────────┐ ┌─────────────────────┐   │
│  │ XUL         │ │ XPCOM       │ │ Firefox API         │   │
│  │ 要素        │ │ サービス     │ │ & コンポーネント      │   │
│  └─────────────┘ └─────────────┘ └─────────────────────┘   │
└─────────────────────────────────────────────────────────────┘
```
...
├── index.ts               # メインAPIエクスポート
├── jsx-runtime.ts         # XUL用カスタムJSXランタイム
├── elements/              # XUL要素ラッパー
│   ├── browser.ts         # ブラウザ固有要素
│   ├── toolbar.ts         # ツールバー要素
│   ├── menu.ts            # メニュー・ポップアップ要素
│   └── common.ts          # 共通XUL要素
├── hooks/                 # XUL/Firefox用SolidJSフック
│   ├── use-xul-element.ts # XUL要素管理
│   ├── use-firefox-api.ts # Firefox API統合
│   └── use-preferences.ts # 設定管理
├── utils/                 # ユーティリティ関数
│   ├── xul-helpers.ts     # XUL操作ヘルパー
│   ├── event-bridge.ts    # イベントシステムブリッジ
│   └── type-guards.ts     # 型判定ユーティリティ
├── types/                 # TypeScript型定義
│   ├── xul.d.ts           # XUL要素型
│   ├── firefox.d.ts       # Firefox API型
│   └── solid-xul.d.ts     # パッケージ固有型
...
メインエントリーポイントは、すべての公開 API とユーティリティをエクスポートします。

```typescript
// Solid-XUL統合のメインエクスポート
...
Solid-XUL 統合パッケージは、現代のリアクティブ UI 開発と Firefox のネイティブ XUL システムとの強力なブリッジを提供し、Floorp が先進的なブラウザ機能を提供する際に Firefox エコシステムとの完全な互換性を維持できるようにします。
