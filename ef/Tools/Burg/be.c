char rcsid_be[] = "$Id: be.c,v 1.1 1998/12/17 06:36:46 fur%netscape.com Exp $";
...
makeLabel()
{
	...
	flag = opsOfArity(0);
	if (flag > 0) {
		fprintf(outfile, "\t\treturn %s_SET_STATE_LABEL(n, %s_state(%s_OP_LABEL(n)",
					prefix, prefix, prefix);
	...
	flag = opsOfArity(1);
	if (flag > 0) {
		fprintf(outfile, "\t\treturn %s_SET_STATE_LABEL(n, %s_state(%s_OP_LABEL(n), %s_label(%s_LEFT_CHILD(n))",
	...
	flag = opsOfArity(2);
	if (flag > 0) {
		fprintf(outfile, "\t\treturn %s_SET_STATE_LABEL(n, %s_state(%s_OP_LABEL(n), %s_label(%s_LEFT_CHILD(n)), %s_label(%s_RIGHT_CHILD(n))",
...
makeCostArray()
{
	...
	for (i = 0; i <= max_erule_num; i++) {
...
#ifdef NOLEX
		if (pVector[i]) {
			fprintf(outfile, "%5d", pVector[i]->rule->delta);
		} else {
		...
		for (j = 0; j < DELTAWIDTH; j++) {
			...
			if (pVector[i]) {
				fprintf(outfile, "%5d", pVector[i]->rule->delta[j]);
			} else {
				fprintf(outfile, "%5d", 0);
			}
		}
...
makeStringArray()
{
	...
	for (i = 0; i <= max_erule_num; i++) {
		if (pVector[i]) {
			fprintf(outfile, "\t\"%s: ", pVector[i]->rule->lhs->name);
...
startOptional()
{
	...
	fprintf(outfile, "#define %s_SET_STATE_LABEL \tSET_STATE_LABEL\n", prefix);
