...
class ProfileBufferChunk {
 public:
  ...
  static MOZ_MUST_USE UniquePtr<ProfileBufferChunk> Create(
    ...
    // UniquePtr for it.
    // `launder` allows the safe conversion of the pointer to `ChunkStruct` to
    // a pointer to `ProfileBufferChunk`, knowing that a valid
    // `ProfileBufferChunk` object was constructed through placement-new.
    UniquePtr<ProfileBufferChunk> chunk{std::launder(
