...
public class NodeImpl_getOwnerDocument extends BWBaseTest implements Execution
{
    ...
    *
    */
   public NodeImpl_getOwnerDocument()
   {
   }
   ...
   public boolean execute(Object tobj)
   {
      ...
      if (d != null)
      {
	    ...
	    if (n.getNodeName().compareTo(nodename) == 0)
	    {
		     ...
		     if (doc == null)
                     {
                       TestLoader.logErrPrint("Could not getOwnerDocument for node " + nodename);
                       return BWBaseTest.FAILED;
                     }
            }
         } else {
