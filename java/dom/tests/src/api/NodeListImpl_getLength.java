...
public class NodeListImpl_getLength extends BWBaseTest implements Execution
{
   ...
   public boolean execute(Object tobj)
   {
      ...
      if (d != null)
      {
             ...
             if (nl == null)
             {
                TestLoader.logErrPrint("Could not Get NodeList ...");
                return BWBaseTest.FAILED;
             } else {
                int len = nl.getLength();
                if (len < 0)
                {
                   TestLoader.logErrPrint("NodeList Length is less than 0");
                   return BWBaseTest.FAILED;
                }
             }
      } else {
