...
/*
 * CurrentPage_getSourceBytes.java
 */
...
public class CurrentPage_getSourceBytes implements Test
{
...
boolean flag=java.lang.Boolean.getBoolean(st.nextToken().trim());
...
if (!url.equals("null")) {
 ...
 if (context.getExecutionCount()!=2) return;
  ...
  try {
	...
	context.setDefaultComment("We are trying to get Source bytes");

	getsource=curPage.getSourceBytes(flag);
	System.out.println("SourceBytes= "+getsource);
	if (!url.equals("null"))
 	 if (getsource.equals(source)) {
	   TestContext.registerPASSED("Right getSourceBytes() value");
	   return;
	 } else {
	   TestContext.registerFAILED("Bad getSourceBytes() value");
	   return;
	 };
