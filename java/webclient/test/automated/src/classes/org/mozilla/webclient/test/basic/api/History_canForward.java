    ...
    public void execute() {      
        ...
        if(context.getExecutionCount()==2){
            ...
            if(currentArg<pages.length-1){
                if(result){
                     TestContext.registerPASSED("canForward returned correct value when it's really can forward");
                     return ;
                 }else{
                     TestContext.registerFAILED("canForward returned false when it's really can forward");
                     return ;
                }
            }else{ 
                if(!result){
                     TestContext.registerPASSED("canForward returned correct value when it's really cannot forward");
                     return ;
                 }else{
                     TestContext.registerFAILED("canForward returned true when it's really cannot forward");
                     return ;
                }
            }
        }
