...
/*
 * EventRegistration_removeDocLoadListener.java
 */
...
public class EventRegistration_removeDocLoadListener implements Test, DocumentLoadListener
{
 ...
 String url=st.nextToken().trim();
  ...
  try {
	...
	case 0:	
		...
		context.setDefaultComment("We are trying to remove null DocumentLoadListener");

		eventReg.removeDocumentLoadListener(null);

		break;
	case 1: 
       		context.setDefaultResult(TestContext.FAILED);
		context.setDefaultComment("We are trying to remove unregistred DocumentLoadListener");

		eventReg.removeDocumentLoadListener(this);
