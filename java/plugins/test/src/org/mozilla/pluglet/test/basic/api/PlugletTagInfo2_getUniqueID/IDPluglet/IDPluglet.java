...
public class IDPluglet implements PlugletFactory {
    public IDPluglet() {
    }
    /**
...
class TestInstance implements Pluglet {
    ...
    public void initialize(PlugletPeer peer) {
      this.peer = peer;
      PlugletTagInfo2 info2 = null;
      PlugletTagInfo info = peer.getTagInfo();
      if (info instanceof PlugletTagInfo2) {
	info2 = (PlugletTagInfo2)info;
      } else {
	TestContext.printLog("Instance of PlugletTagInfo instead of PlugletTagInfo2 passed,\n" +
			     "can't call getUniqueID.");
	return;
      }
      int ID = info2.getUniqueID();
      org.mozilla.pluglet.test.basic.api.PlugletTagInfo2_getUniqueID_0.addID(ID);
    }
    /**
