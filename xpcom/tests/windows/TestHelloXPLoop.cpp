...
int WINAPI WinMain(HINSTANCE inst, HINSTANCE prevInstance, LPSTR lpszCmdLine,
  ...
  {  //  Needed to scope all nsCOMPtr within XPCOM Init and Shutdown
    ...
    wnd = CreateWindow(lpszAppName, "The Hello World", WS_OVERLAPPEDWINDOW,
                       CW_USEDEFAULT, CW_USEDEFAULT, CW_USEDEFAULT,
                       CW_USEDEFAULT, nullptr, nullptr, inst, nullptr);
    ...
    if (NS_FAILED(nativeAppService->CreateEventLoop(
            L"_MainLoop", nsEventLoopTypes::MainAppLoop,
...
LRESULT CALLBACK WndProc(HWND wnd, UINT msg, WPARAM wParam, LPARAM lParam) {
  ...
  switch (msg) {
    ...
    case WM_DESTROY: {
      ...
      if (NS_FAILED(nativeAppService->FindEventLoop(
              L"_MainLoop", getter_AddRefs(eventLoop)))) {
