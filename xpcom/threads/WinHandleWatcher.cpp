...
//
// Throughout this class, we typically use manual locking and unlocking guarded
// by Clang's thread-safety warnings, rather than scope-based lock-guards. See
// `Replace()` for an explanation and justification.
//
...
class HandleWatcher::Impl final : public nsITargetShutdownTask {
 ...
 private:
  ...
  void TargetShutdown() MOZ_EXCLUDES(mMutex) override final {
    ...
    mData.target = nullptr;  // no need to unregister
    Replace(Data{});
  }
 ...
 private:
  // Throughout this class, we typically use manual locking and unlocking
  // guarded by Clang's thread-safety warnings, rather than scope-based
  // lock-guards. This is largely driven by `Replace()`, below, which performs
  // both operations which require the mutex to be held and operations which
  // require it to not be held, and therefore must explicitly sequence the mutex
  // release.
  //
  ...
  void Replace(Data&& aData) MOZ_CAPABILITY_RELEASE(mMutex) {
    ...
    // Extract current `self` and `waitHandle` for manipulation after releasing
    // the mutex.
    RefPtr<Impl> self = std::move(mData.self);
    WaitHandlePtr waitHandle = std::move(mData.waitHandle);
    ...
    // If `self` has been unset, the callback has already run. Otherwise, we
    // need to clear out the wait-object first.
    if (self) {
      MOZ_ASSERT(waitHandle);
      FlushWaitHandle(waitHandle.get());
    }
  }
  ...
  void FlushWaitHandle(PTP_WAIT aWaitHandle) MOZ_EXCLUDES(mMutex) {
    ...
    // ::CloseThreadpoolWait() is the caller's responsibility.
  }
};
