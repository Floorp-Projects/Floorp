...
struct nsCycleCollectorStats
{
    ...
    PRUint32 mSuspectNewNode;
    PRUint32 mForgotPointer;
...
SufficientlyAged(PRUint32 generation, nsCycleCollector *collector)
{
    return generation + collector->mParams.mScanDelay < collector->mCurrGen;
}
             ...
             void*        userArg)
{
    nsCycleCollector *collector = NS_STATIC_CAST(nsCycleCollector*, 
                                                 userArg);
    if (SufficientlyAged(generation, collector)) {
...
{
    mBufs[0]->Empty();
    mPurpleBuf.Enumerate(markCallback, this);
}
...
zeroGenerationCallback(const void*  ptr,
                       ...
                       void*        userArg)
{
    sCollector.mStats.mZeroGeneration++;
    generation = 0;
    return PL_DHASH_NEXT;
}
...
void 
nsCycleCollector::BumpGeneration()
{
    if (mCurrGen == 0xffffffff) {
        mPurpleBuf.Enumerate(zeroGenerationCallback, this);
        mCurrGen = 0;
    } else {
        ++mCurrGen;
    }
...
nsCycleCollector::Suspect(nsISupports *n)
{
    ...
    if (!mPurpleBuf.Get(n, &gen)) {
        hit = PR_TRUE;
        mStats.mSuspectNewNode++;
        mPurpleBuf.Put(n, mCurrGen);
    }

    if (mParams.mLogPointers) {
        if (!mPtrLog)
            mPtrLog = fopen("pointer_log", "w");
        fprintf(mPtrLog, "S %p %d\n", NS_STATIC_CAST(void*, n), hit);
    }
}
