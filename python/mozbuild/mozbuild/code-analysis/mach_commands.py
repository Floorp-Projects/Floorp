...
class PackageFrontend(MachCommandBase):
    ...
    @CommandArgument(
        "--tooltool-url", metavar="URL", help="Use the given url as tooltool server"
    )
    @CommandArgument(
    ...
    def artifact_toolchain(
        ...
        tooltool_url = (tooltool_url or "https://tooltool.mozilla-releng.net").rstrip(
            "/"
        )
        ...
        if authentication_file:
            ...
            cache._download_manager.session.mount(tooltool_url, TooltoolAuthenticator())
        ...
        if from_build:
            ...
            params = Parameters(
                level=os.environ.get("MOZ_SCM_LEVEL", "3"), strict=False
            )
...
class StaticAnalysis(MachCommandBase):
    ...
    # List of file extension to consider (should start with dot)
    _format_include_extensions = (".cpp", ".c", ".cc", ".h", ".m", ".mm")
    # File containing all paths to exclude from formatting
    _format_ignore_file = ".clang-format-ignore"
    ...
    def check_coverity(
        ...
        # For each element in commands_list run `cov-translate`
        for element in commands_list:
            cmd = [self.cov_translate, "--dir", self.cov_idir_path] + element[
                "command"
            ].split(" ")
            self.log(
                logging.INFO,
                "static-analysis",
                {},
                "Running Coverity Translate for {}".format(cmd),
            )
            rc = self.run_process(args=cmd, cwd=element["directory"], pass_thru=True)
            if rc != 0:
                self.log(
                    ...
                    "static-analysis",
                    {},
                    "Running Coverity Translate failed for {}".format(cmd),
                )
                return cmd
    ...
    def dump_cov_artifact(self, cov_results, source, output):
        ...
        with open(cov_results) as f:
            ...
            for issue in result["issues"]:
                path = self.cov_is_file_in_source(
                    issue["strippedMainEventFilePathname"], source
                )
                if path is None:
                    # Since we skip a result we should log it
                    self.log(
    ...
    def download_coverity(self):
        if (
            ...
            self.log(
                ...
                "static-analysis",
                {},
                "Missing Coverity secret on try job!",
            )
            return 1
    ...
    def check_java(
        ...
        all_checkers, third_party_path = self._get_infer_config()
        checkers, excludes = self._get_infer_args(
            checks=checks or all_checkers, third_party_path=third_party_path
        )
        rc = rc or self._gradle(["clean"])  # clean so that we can recompile
        # infer capture command
        capture_cmd = [self._infer_path, "capture"] + excludes + ["--"]
    ...
    def _is_version_eligible(self):
        ...
        try:
            ...
            self.log(
                ...
                "static-analysis",
                {},
                "Error determining the version clang-tidy/format binary, "
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=max_workers) as executor:
            ...
            if error_code != self.TOOLS_SUCCESS:
                ...
                for failure in checkers_results:
                    ...
                    message_to_log = ""
                    if checker_error == self.TOOLS_CHECKER_NOT_FOUND:
                        message_to_log = "\tChecker {} not present in this "
                        "clang-tidy version.".format(
                            checker_name
                        )
                    elif checker_error == self.TOOLS_CHECKER_NO_TEST_FILE:
                        message_to_log = "\tChecker {0} does not have a test file - "
                        "{0}.cpp".format(
                            checker_name
                        )
                    elif checker_error == self.TOOLS_CHECKER_RETURNED_NO_ISSUES:
                        message_to_log = "\tChecker {0} did not find any issues in its "
                        "test file, clang-tidy output for the run is:\n{1}".format(
                            checker_name, info1
                        )
                    elif checker_error == self.TOOLS_CHECKER_RESULT_FILE_NOT_FOUND:
                        message_to_log = "\tChecker {0} does not have a result file - "
                        "{0}.json".format(
                            checker_name
                        )
                    elif checker_error == self.TOOLS_CHECKER_DIFF_FAILED:
    ...
    def _run_analysis_batch(self, items):
        ...
        if len(failed_checks) > 0:
            self.log(
                ...
                "static-analysis",
                {},
                "The following check(s) failed for bulk analysis: "
                + " ".join(failed_checks),
            )
    ...
    def _create_temp_compilation_db(self, config):
        directory = tempfile.mkdtemp(prefix="cc")
        with open(
            mozpath.join(directory, "compile_commands.json"), "wb"
        ) as file_handler:
            compile_commands = []
            director = mozpath.join(self.topsrcdir, "tools", "clang-tidy", "test")
            for item in config["clang_checkers"]:
    ...
    def _verify_infer_checker(self, item):
        ...
        if not issues:
            self.log(
                ...
                "static-analysis",
                {},
                "ERROR: infer check {0} did not find any issues in its "
                "associated test suite.".format(
                    check
                ),
            )
            return self.TOOLS_CHECKER_RETURNED_NO_ISSUES
        if self._dump_results:
            ...
            if ordered(issues) != ordered(baseline_issues):
                ...
                self.log(
                    ...
                    "static-analysis",
                    {},
                    "ERROR: infer autotest for check {} failed, check stdout for "
                    "more details".format(
                        check
                    ),
                )
    ...
    def clang_format(
        ...
        # With assume_filename we want to have stdout clean since the result of the
        # format will be redirected to stdout. Only in case of error we
        # write something to stdout.
        # We don't actually want to get the clang-tools here since we want in some
        # scenarios to do this in parallel so we relay on the fact that the tools
        # have already been downloaded via './mach bootstrap' or directly via
        ...
        if output_file:
            ...
            for i in range(0, len(path_list)):
                ...
                try:
                    output = check_output(args + l)
                    if output and output_format == "json":
                        patches[original_path] = self._parse_xml_output(
                ...
                if output_format == "diff":
                    ...
                    try:
                        ...
                        # there is a diff to show
                        if e.output:
                            # Replace the temp path by the path relative to the repository
                            # to display a valid patch
    ...
    def _parse_xml_output(self, path, clang_output):
        """
        Parse the clang-format XML output to convert it in a JSON compatible
        list of patches, and calculates line level information from the
        character level provided changes.
        """
        content = open(path, "r").read().decode("utf-8")
