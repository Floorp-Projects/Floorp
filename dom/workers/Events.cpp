...
public:
  ...
  Create(JSContext* aCx, JSObject* aParent, JSString* aMessage,
         JSString* aFilename, uint32 aLineNumber)
  {
...
Event::GetPrivate(JSContext* aCx, JSObject* aObj)
{
  if (aObj) {
    ...
    if (classPtr == &sClass ||
        classPtr == MessageEvent::Class() ||
        classPtr == ErrorEvent::Class() ||
...
{
  JSObject* eventProto = Event::InitClass(aCx, aGlobal, NULL);
  if (!eventProto) {
    return false;
  }

  return MessageEvent::InitClass(aCx, aGlobal, eventProto) &&
         ErrorEvent::InitClass(aCx, aGlobal, eventProto) &&
                   ...
                   bool aCancelable)
{
  ...
  return Event::Create(aCx, global, aType, aBubbles, aCancelable);
}
...
{
  JSObject* global = JS_GetGlobalObject(aCx);
  return MessageEvent::Create(aCx, global, aData);
}
