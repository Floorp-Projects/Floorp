...
QuotaClient::FindDatabasesForIdleMaintenance(uint32_t aRunId)
{
  ...
  bool exists;
  bool isDirectory;
  if (NS_FAILED(storageDir->Exists(&exists)) ||
      !exists ||
      NS_FAILED(storageDir->IsDirectory(&isDirectory)) ||
      !isDirectory) {
    return;
  }
  ...
  for (const PersistenceType persistenceType : kPersistenceTypes) {
    ...
    if (NS_FAILED(persistenceDir->Exists(&exists)) ||
        !exists ||
        NS_FAILED(persistenceDir->IsDirectory(&isDirectory)) ||
        !isDirectory) {
      continue;
    }

    nsCOMPtr<nsISimpleEnumerator> persistenceDirEntries;
    if (NS_FAILED(persistenceDir->GetDirectoryEntries(
    ...
    while (true) {
      ...
      if (NS_FAILED(persistenceDirEntries->HasMoreElements(
                      &persistenceDirHasMoreEntries)) ||
          !persistenceDirHasMoreEntries ||
          NS_FAILED(persistenceDirEntries->GetNext(
                      getter_AddRefs(persistenceDirEntry)))) {
        break;
      }
      ...
      if (NS_FAILED(originDir->Exists(&exists)) ||
          !exists ||
          NS_FAILED(originDir->IsDirectory(&isDirectory)) ||
          !isDirectory) {
        continue;
      }
      ...
      if (NS_FAILED(idbDir->Exists(&exists)) ||
          !exists ||
          NS_FAILED(idbDir->IsDirectory(&isDirectory)) ||
          !isDirectory) {
        continue;
      }

      nsCOMPtr<nsISimpleEnumerator> idbDirEntries;
      if (NS_FAILED(idbDir->GetDirectoryEntries(
                      getter_AddRefs(idbDirEntries))) ||
          !idbDirEntries) {
        continue;
      }
      ...
      while (true) {
        ...
        if (NS_FAILED(idbDirEntries->HasMoreElements(&idbDirHasMoreEntries)) ||
            !idbDirHasMoreEntries ||
            NS_FAILED(idbDirEntries->GetNext(getter_AddRefs(idbDirEntry)))) {
          break;
        }
        ...
        if (NS_FAILED(idbDirFile->Exists(&exists)) ||
            !exists ||
            NS_FAILED(idbDirFile->IsDirectory(&isDirectory)) ||
            isDirectory) {
          continue;
        }
                                  ...
                                  const SingleMaintenanceInfo& aMaintenanceInfo)
{
  ...
  bool exists;
  if (NS_FAILED(databaseFile->Exists(&exists)) || !exists) {
    return;
  }
