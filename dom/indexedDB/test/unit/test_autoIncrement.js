...
function* testSteps() {
  ...
  store1.add({ foo: "value1" }).onsuccess = genCheck(
    c1++,
    { foo: "value1" },
    "first" + test
  );
  store1.add({ foo: "value2" }).onsuccess = genCheck(
    c1++,
    { foo: "value2" },
    "second" + test
  );
  ...
  store2.put({ bar: "value1" }).onsuccess = genCheck(
    c2,
    { bar: "value1", id: c2 },
    "first in store2" + test,
    { store: store2 }
  );
  c2++;
  store1.put({ foo: "value3" }).onsuccess = genCheck(
    c1++,
    { foo: "value3" },
    "third" + test,
    { store: store1 }
  ...
  yield undefined;
  trans.objectStore("store1").add({ explicit: 6 }).onsuccess = genCheck(
    c1++,
    { explicit: 6 },
    "sixth" + test
  );
  ...
  trans = db.transaction("store1", RW);
  trans.objectStore("store1").add({ explicit: 7 }, "key").onsuccess = genCheck(
    "key",
    { explicit: 7 },
    "seventh" + test
  );
  ...
  trans2.objectStore("store1").put({ over: 2 }).onsuccess = genCheck(
    c1 + 1,
    { over: 2 },
    "first" + test,
    { trans: trans2 }
  );
  trans.objectStore("store1").put({ over: 1 }).onsuccess = genCheck(
    c1,
    { over: 1 },
    "second" + test,
    { trans }
  ...
  trans2.objectStore("store2").put({ over: 2 }).onsuccess = genCheck(
    c2 + 1,
    { over: 2, id: c2 + 1 },
    "third" + test,
    { trans: trans2 }
  );
  trans.objectStore("store2").put({ over: 1 }).onsuccess = genCheck(
    c2,
    { over: 1, id: c2 },
    "fourth" + test,
    { trans }
  ...
  trans
    .objectStore("store1")
    .add({ error: 1, unique: 1 })
    .addEventListener("error", new ExpectError("ConstraintError", true));
  trans.objectStore("store1").add({ error: 2 }).onsuccess = genCheck(
    c1++,
    { error: 2 },
    "first" + test
  );
