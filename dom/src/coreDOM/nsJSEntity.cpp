...
GetEntityProperty(JSContext *cx, JSObject *obj, jsval id, jsval *vp)
{
  ...
  if (JSVAL_IS_INT(id)) {
    ...
    switch(JSVAL_TO_INT(id)) {
      ...
      {
        PRBool ok = PR_FALSE;
        secMan->CheckScriptAccess(scriptCX, obj, NS_DOM_PROP_ENTITY_PUBLICID, PR_FALSE, &ok);
        if (!ok) {
      ...
      {
        PRBool ok = PR_FALSE;
        secMan->CheckScriptAccess(scriptCX, obj, NS_DOM_PROP_ENTITY_SYSTEMID, PR_FALSE, &ok);
        if (!ok) {
          return nsJSUtils::nsReportError(cx, NS_ERROR_DOM_SECURITY_ERR);
        }
        nsAutoString prop;
      ...
      {
        PRBool ok = PR_FALSE;
        secMan->CheckScriptAccess(scriptCX, obj, NS_DOM_PROP_ENTITY_NOTATIONNAME, PR_FALSE, &ok);
        if (!ok) {
          return nsJSUtils::nsReportError(cx, NS_ERROR_DOM_SECURITY_ERR);
        }
        nsAutoString prop;
