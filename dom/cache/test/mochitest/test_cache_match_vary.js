...
function testStar() {
  var test;
  return setupTest({"WhatToVary": "*", "Custom-Cookie": "foo=bar"})
    .then(function(t) {
      test = t;
      // Ensure that searching with a different Cookie header with Vary:* succeeds.
      return test.cache.match(new Request(requestURL, {headers: {"Custom-Cookie": "bar=baz"}}));
    }).then(function(r) {
      return checkResponse(r, test.response, test.responseText);
    });
}
...
function testStarAndAnotherHeader() {
  var test;
  return setupTest({"WhatToVary": "*,Custom-User-Agent"})
    .then(function(t) {
      test = t;
      // Ensure that searching with a different Custom-User-Agent header fails.
