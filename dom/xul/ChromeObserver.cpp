...
void ChromeObserver::SetChromeMargins(const nsAttrValue* aValue) {
  ...
  // If the widget isn't visible yet, don't set the chrome margin to avoid
  // triggering widget code when it isn't ready (bug 1598848). If a
  // chromemargin is set, AppWindow::SyncAttributesToWidget will handle applying
  // the margin.
  if (!mainWidget->IsVisible()) return;
