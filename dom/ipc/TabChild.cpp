...
nsresult TabChild::CanCancelContentJS(
  ...
  *aCanCancel = false;
  ...
  if (aNavigationType == nsITabParent::NAVIGATE_BACK) {
    aNavigationIndex = current - 1;
  } else if (aNavigationType == nsITabParent::NAVIGATE_FORWARD) {
    aNavigationIndex = current + 1;
  } else if (aNavigationType == nsITabParent::NAVIGATE_URL) {
    if (!aNavigationURI) {
      return NS_ERROR_FAILURE;
    }
...
nsresult TabChild::CanCancelContentJSBetweenURIs(nsIURI* aFirstURI,
                                                 nsIURI* aSecondURI,
                                                 bool* aCanCancel) {
  nsresult rv;
  *aCanCancel = false;
