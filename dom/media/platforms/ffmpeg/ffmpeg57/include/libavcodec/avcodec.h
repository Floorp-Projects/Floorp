 ...
 * user applications. sizeof(AVCodecContext) must not be used outside libav*.
 */
...
typedef struct AVCodec {
  ...
  uint8_t max_lowres;    ///< maximum value for lowres supported by the decoder,
                         ///< no direct access, use av_codec_get_max_lowres()
 ...
 * @param filter_length    length of each FIR filter in the filterbank relative
 ...
 * @param linear           if 1 then the used FIR filter will be linearly
 interpolated between the 2 closest, if 0 the closest will be used
 ...
 * Note, if either rate is not an integer then simply scale both rates up so
 they are.
 * @param filter_length length of each FIR filter in the filterbank relative to
 the cutoff freq
 * @param log2_phase_count log2 of the number of entries in the polyphase
 filterbank
 ...
 * @param consumed the number of samples of src which have been consumed are
 * returned here
 * @param src_size the number of unconsumed samples available
 * @param dst_size the amount of space in samples available in dst
 * @param update_ctx If this is 0 then the context will not be modified, that
 ...
 * especially if the compensation_distance is large and the in_rate used during
 * init is small
 */
 ...
 * av_pix_fmt_get_chroma_sub_sample for one that returns a failure code and
 * continues in case of invalid pix_fmts.
 *
 ...
 * of the output stream the packet is sent to.
 * @param args            arguments which specify the filter configuration, may
 * be NULL
 * @param poutbuf         pointer which is updated to point to the filtered
 * buffer
 * @param poutbuf_size    pointer which is updated to the filtered buffer size
 * in bytes
 * @param buf             buffer containing the data to filter
 * @param buf_size        size in bytes of buf
 * @param keyframe        set to non-zero if the buffer to filter corresponds to
 * a key-frame packet data
 ...
 * feature. If want_sample is non-zero, additional verbage will be added to the
 * log message which tells the user how to report samples to the development
 * mailing list.
