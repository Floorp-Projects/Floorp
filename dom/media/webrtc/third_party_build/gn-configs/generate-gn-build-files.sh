...
DO_BOOTSTRAP=1
if [ ! "x$MOZ_SKIP_LINUX_BOOTSTRAP" = "x" ] &&
   [ "$MOZ_SKIP_LINUX_BOOTSTRAP" = "1" ]; then
  echo "MOZ_SKIP_LINUX_BOOTSTRAP=1 detected, skipping bootstrap"
  DO_BOOTSTRAP=0
fi
...
SYS_NAME=`uname`
IS_UBUNTU=`uname -v | grep -i ubuntu | wc -l`

if [ "x$SYS_NAME" = "xLinux" ] && [ "x$MOZ_GEN_X86" = "x" ]; then
  echo "For linux generation, please indicate whether x86 json files should be"
  echo "generated by including MOZ_GEN_X86=1 or MOZ_GEN_X86=0 before the command."
  echo "** Note: when generating x86 json files, Ubuntu is expected, and it will"
  echo "         be necessary to install i386 versions of some libraries."
  echo "** Note: you will be prompted for a sudo password when generating x86"
  echo "         files unless adding MOZ_SKIP_LINUX_BOOTSTRAP=1 to the cmdline."
  exit
fi

# After this point, add:
# * u: All variables should be defined before use.
set -u

if [ "x$SYS_NAME" = "xLinux" ]; then
  if [ "$MOZ_GEN_X86" != "0" ] && [ "$MOZ_GEN_X86" != "1" ] ; then
    echo "Error: MOZ_GEN_X86 should be set to 1 or 0"
    exit
  fi
  if [ "$MOZ_GEN_X86" = "1" ] && [ ! 1 -eq $IS_UBUNTU ]; then
    echo "Error: Ubuntu is not detected for x86 linux generation."
    exit
  fi
fi
...
if [ "x$SYS_NAME" = "xDarwin" ]; then
  ...
  if [ "$MOZ_GEN_X86" = "1" ] ; then
    CONFIGS="x64_False_x86_linux x64_True_x86_linux"

    if [ $DO_BOOTSTRAP -eq 1 ]; then
      rustup target add i686-unknown-linux-gnu
      sudo dpkg --add-architecture i386
      sudo apt-get update
      sudo apt-get install -y libharfbuzz-dev:i386 gcc-multilib g++-multilib \
           libdbus-glib-1-dev:i386 libgtk2.0-dev:i386 libgtk-3-dev:i386 \
           libpango1.0-dev:i386 libxt-dev:i386 libpulse-dev:i386 libdrm-dev:i386 \
           libx11-xcb-dev:i386
    fi
  else
