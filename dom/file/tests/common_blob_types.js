let blobTypes = [
  { type: 'memory',
    factory: async content => { return new Blob([content]); },
  ...
  { type: 'ipcBlob',
    ...
    blobImplType: "StreamBlobImpl[StreamBlobImpl[MultipartBlobImpl[StringBlobImpl]]]"
  },
...
async function forEachBlobType(content, cb) {
  for (let i = 0; i < blobTypes.length; ++i) {
    ...
    is(SpecialPowers.wrap(blob).blobImplType, blobTypes[i].blobImplType, "Correct blobImplType");
