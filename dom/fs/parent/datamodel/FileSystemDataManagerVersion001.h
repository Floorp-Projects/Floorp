 ...
 * - a new implementation FileSystemDataManagerVersion00x is derived from the
 * previous version and the required methods are overridden
 * - a new apppropriate schema initialization class SchemaVersion00x is created
 * or derived
 * - the factory method of FileSystemDataManager is extended to try to
 * migrate the data from the previous version to version x, and to return
 * FileSystemDataManagerVersion00x implementation if the database version after
 * the migrations is x
...
#endif  // DOM_FS_PARENT_FILESYSTEMDATAMANAGERVERSION001_H_
