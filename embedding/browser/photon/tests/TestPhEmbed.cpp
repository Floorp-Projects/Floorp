...
int reload_cb(PtWidget_t *widget, void *data, PtCallbackInfo_t *cbinfo)
{
	...
	PtGetResource(PtWidgetParent(widget), Pt_ARG_POINTER, &info, 0);
...
int print_cb(PtWidget_t *widget, void *data, PtCallbackInfo_t *cbinfo)
{
	...
	struct window_info *info;
	PtGetResource(PtWidgetParent(widget), Pt_ARG_POINTER, &info, 0);
	PtSetArg(&args[0], Pt_ARG_MOZ_PRINT, 0, 0);
	PtSetResources(info->web, 1, args);
    return (Pt_CONTINUE);
}
...
int select_cb(PtWidget_t *widget, void *data, PtCallbackInfo_t *cbinfo)
{
	PtArg_t args[1];
	struct window_info *info;

	PtGetResource(PtWidgetParent(widget), Pt_ARG_POINTER, &info, 0);
	PtSetArg(&args[0], Pt_ARG_MOZ_COMMAND, Pt_MOZ_COMMAND_SELECTALL, 0);
	PtSetResources(info->web, 1, args);
...
int save_cb(PtWidget_t *widget, void *data, PtCallbackInfo_t *cbinfo)
{
	...
	PtFileSelection(info->window, NULL, "Save Page As", home, "*.html,*.txt", \
		NULL, NULL, NULL, &i, Pt_FSR_NO_FCHECK);

	if (strstr(i.path, ".html"))
		MozSavePageAs(info->web, i.path, Pt_MOZ_SAVEAS_HTML);
	else
		MozSavePageAs(info->web, i.path, Pt_MOZ_SAVEAS_TEXT);
...
int moz_info_cb(PtWidget_t *widget, void *data, PtCallbackInfo_t *cbinfo)
{
	...
	PtGetResource(PtWidgetParent(widget), Pt_ARG_POINTER, &i, 0);
...
int moz_open_cb(PtWidget_t *widget, void *data, PtCallbackInfo_t *cbinfo)
{
	...
	{
		PtAskQuestion(NULL, "Open URL Callback", "You can't go to google.", \
			NULL, "OK", NULL, NULL, 1);
		return (Pt_END);
	}
...
int moz_auth_cb(PtWidget_t *widget, void *data, PtCallbackInfo_t *cbinfo)
{
	...
	return (CreateAuthWindow(a, PtWidgetParent(widget)));
}
...
{
	PtDestroyWidget(PtWidgetParent(widget));
	return (Pt_CONTINUE);
}
...
PtWidget_t *create_browser_window(unsigned window_flags)
{
	...
	//	if (window_flags & Pt_MOZ_FLAG_WINDOWRESIZEON)
		...
		info->back = PtCreateWidget(PtButton, info->window, n, args);
		...
		info->forward = PtCreateWidget(PtButton, info->window, n, args);
		...
		info->reload = PtCreateWidget(PtButton, info->window, n, args);
		...
		info->stop = PtCreateWidget(PtButton, info->window, n, args);
		...
		info->print = PtCreateWidget(PtButton, info->window, n, args);
		...
		area.pos.x += 71;
		PtSetArg(&args[n++], Pt_ARG_AREA, &area, 0);
		PtSetArg(&args[n++], Pt_ARG_TEXT_STRING, "Select", 0);
		info->sel = PtCreateWidget(PtButton, info->window, n, args);
		PtAddCallback(info->sel, Pt_CB_ACTIVATE, select_cb, NULL);
		...
		info->save = PtCreateWidget(PtButton, info->window, n, args);
	...
	if (window_flags & Pt_MOZ_FLAG_LOCATIONBARON)
	{
		...
		PtSetArg(&args[n++], Pt_ARG_ANCHOR_FLAGS, ~0, Pt_RIGHT_ANCHORED_RIGHT|Pt_LEFT_ANCHORED_LEFT| \
				Pt_TOP_ANCHORED_TOP);
		info->url = PtCreateWidget(PtText, info->window, n, args);
	...
	if (window_flags & Pt_MOZ_FLAG_STATUSBARON)
	{
		...
		info->status = PtCreateWidget(PtLabel, info->window, n, args);
		...
		PtSetArg(&args[n++], Pt_ARG_ANCHOR_FLAGS, ~0, Pt_RIGHT_ANCHORED_RIGHT|Pt_LEFT_ANCHORED_RIGHT| \
				Pt_TOP_ANCHORED_BOTTOM);
		info->progress = PtCreateWidget(PtProgress, info->window, n, args);
  	}
	...
	area.size.h = win_dim.h;
	if ((window_flags & Pt_MOZ_FLAG_LOCATIONBARON) || (window_flags & Pt_MOZ_FLAG_TOOLBARON))
	{
		area.pos.y += 26;
		area.size.h -= 26;
	}
	...
	n = 0;
	PtSetArg(&args[n++], Pt_ARG_AREA, &area, 0);
	PtSetArg(&args[n++], Pt_ARG_ANCHOR_FLAGS, ~0, Pt_RIGHT_ANCHORED_RIGHT | \
			Pt_LEFT_ANCHORED_LEFT | Pt_TOP_ANCHORED_TOP | Pt_BOTTOM_ANCHORED_BOTTOM);
  	info->web = PtCreateWidget(PtMozilla, info->window, n, args);
...
void set_pref(PtWidget_t *widget)
{
	// see 'mozilla/modules/libpref/src/init/all.js' for a list of preferences
	MozSetPreference(widget, Pt_MOZ_PREF_BOOL, "browser.underline_anchors", (void *)0);
	MozSetPreference(widget, Pt_MOZ_PREF_COLOR, "browser.anchor_color", (void *)0xff0000);
}
/////////////////////////////////////////////////////////////////////////////
