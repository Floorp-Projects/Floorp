...
/* void noParamNoResult (); */
static JSBool
nsIMessenger_noParamNoResult(JSContext *cx, JSObject *obj, uintN argc, jsval *argv, jsval *rval)
{
  ...
  if (!JS_ConvertArguments(cx, argc, argv, ""))
    return JS_FALSE;
  nsresult result = priv->noParamNoResult();
  if (NS_FAILED(result)) {
...
/* void ParamNoResult (in long i); */
static JSBool
nsIMessenger_ParamNoResult(JSContext *cx, JSObject *obj, uintN argc, jsval *argv, jsval *rval)
{
  nsIMessenger *priv = (nsIMessenger *)JS_GetPrivate(cx, obj);
  if (!priv)
    return JS_TRUE;
  PRInt32 i;
  if (!JS_ConvertArguments(cx, argc, argv, "i", &i))
    return JS_FALSE;
  nsresult result = priv->ParamNoResult(i);
  if (NS_FAILED(result)) {
    JS_ReportError(cx, XXXnsresult2string(result));
    return JS_FALSE;
  }
...
/* long noParamResult (); */
static JSBool
nsIMessenger_noParamResult(JSContext *cx, JSObject *obj, uintN argc, jsval *argv, jsval *rval)
{
  nsIMessenger *priv = (nsIMessenger *)JS_GetPrivate(cx, obj);
  if (!priv)
    return JS_TRUE;
  if (!JS_ConvertArguments(cx, argc, argv, ""))
    return JS_FALSE;
  PRInt32 retval;
  nsresult result = priv->noParamResult(&retval);
  if (NS_FAILED(result)) {
    JS_ReportError(cx, XXXnsresult2string(result));
    return JS_FALSE;
  }
  if (!JS_NewNumberValue(cx, (jsdouble) retval, rval))
    return JS_FALSE;
  return JS_TRUE;
}

/* long ParamResult (in long i); */
static JSBool
nsIMessenger_ParamResult(JSContext *cx, JSObject *obj, uintN argc, jsval *argv, jsval *rval)
{
  nsIMessenger *priv = (nsIMessenger *)JS_GetPrivate(cx, obj);
  if (!priv)
    return JS_TRUE;
  ...
  if (!JS_ConvertArguments(cx, argc, argv, "i", &i))
    return JS_FALSE;
  PRInt32 retval;
  nsresult result = priv->ParamResult(i, &retval);
  if (NS_FAILED(result)) {
    JS_ReportError(cx, XXXnsresult2string(result));
    return JS_FALSE;
  }
...
static JSFunctionSpec nsIMessenger_funcs[] = {
  {"noParamNoResult", nsIMessenger_noParamNoResult, 0},
  {"ParamNoResult", nsIMessenger_ParamNoResult, 1},
  {"noParamResult", nsIMessenger_noParamResult, 0},
  {"ParamResult", nsIMessenger_ParamResult, 1},
  {0}
};
