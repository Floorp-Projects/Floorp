...
static char *msg_extract_rfc822_address_names(PRInt16 csid, const char *line);
static char *msg_extract_rfc822_address_name(PRInt16 csid, const char *line);
static char *msg_format_rfc822_addresses(const char *names, const char *addrs, int count,
                                         PRBool wrap_lines_p);
static char *msg_reformat_rfc822_addresses(PRInt16 csid, const char *line);
...
/*
 * nsMsgRFC822Parser definitions....
 */

nsMsgRFC822Parser::nsMsgRFC822Parser()
{
...
nsresult nsMsgRFC822Parser::ExtractRFC822AddressMailboxes (const char *line, char ** mailboxes)
{
	if (mailboxes)
	{
		*mailboxes = msg_extract_rfc822_address_mailboxes(CS_APP_DEFAULT, line);
		return NS_OK;
	}
	else
...
nsresult nsMsgRFC822Parser::ExtractRFC822AddressNames (const char *line, char ** names)
{
	if (names)
	{
		*names = msg_extract_rfc822_address_names(CS_APP_DEFAULT, line);
		return NS_OK;
	}
	else
		return NS_ERROR_NULL_POINTER;
...
nsresult nsMsgRFC822Parser::ExtractRFC822AddressName (const char *line, char ** name)
{
	if (name)
	{
		*name = msg_extract_rfc822_address_name(CS_APP_DEFAULT, line);
		return NS_OK;
	}
	else
		return NS_ERROR_NULL_POINTER;
...
nsresult nsMsgRFC822Parser::ReformatRFC822Addresses (const char *line, char ** reformattedAddress)
{
	...
	{
		*reformattedAddress = msg_reformat_rfc822_addresses(CS_APP_DEFAULT, line);
		return NS_OK;
	}
	else
		return NS_ERROR_NULL_POINTER;
...
/* msg_extract_rfc822_address_mailboxes
 *
 * Given a string which contains a list of RFC822 addresses, returns a
 * comma-seperated list of just the `mailbox' portions.
 */
static char *
msg_extract_rfc822_address_mailboxes(PRInt16 csid, const char *line)
{
	char *addrs = 0;
	char *result, *s, *out;
	PRUint32 i, size = 0;
	int status = msg_parse_rfc822_addresses(csid, line, NULL, &addrs);
	if (status <= 0)
		return NULL;
...
/* msg_extract_rfc822_address_names
 *
 * Given a string which contains a list of RFC822 addresses, returns a
 ...
 */
static char *
msg_extract_rfc822_address_names(PRInt16 csid, const char *line)
{
	char *names = 0;
	char *addrs = 0;
 ...
 */
static char *
msg_reformat_rfc822_addresses(PRInt16 csid, const char *line)
{
	char *names = 0;
	char *addrs = 0;
	char *result;
                               ...
                               PRBool removeAliasesToMe)
{
	...
	count1 = msg_parse_rfc822_addresses(csid, addrs, &names1, &addrs1);
	...
	if (other_addrs)
		count2 = msg_parse_rfc822_addresses(csid, other_addrs, &names2, &addrs2);
