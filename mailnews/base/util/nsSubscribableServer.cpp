...
nsresult
nsMsgIncomingServer::SetAsSubscribedInSubscribeDS(const char *aName)
{
    nsresult rv;
...
nsresult
nsMsgIncomingServer::UpdateSubscribedInSubscribeDS()
{
	nsresult rv;
...
nsresult
nsMsgIncomingServer::AddToSubscribeDS(const char *aName)
{
	nsresult rv;
...
nsresult
nsMsgIncomingServer::SetPropertiesInSubscribeDS(const char *uri, const char *aName, nsIRDFResource *aResource)
{
	nsresult rv;
...
nsresult
nsMsgIncomingServer::FindAndAddParentToSubscribeDS(const char *uri, const char *serverUri, const char *aName, nsIRDFResource *aChildResource)
{
	nsresult rv;
...
#if 0 /* news */
	PRInt32 startpos = uriCStr.RFindChar('/',PR_TRUE);
	PRInt32 delimpos = uriCStr.RFindChar('.',PR_TRUE);
#else
...
#if 0 /* news */
		PRInt32 namedelimpos = nameCStr.RFindChar('.',PR_TRUE);
#else
		PRInt32 namedelimpos = nameCStr.RFindChar('/',PR_TRUE);
#endif
...
nsresult
nsMsgIncomingServer::StopPopulatingSubscribeDS()
{
	mRDFService = nsnull;
...
nsresult
nsMsgIncomingServer::StartPopulatingSubscribeDS()
{
  nsresult rv;
