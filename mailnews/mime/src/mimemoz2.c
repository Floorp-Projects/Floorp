...
mime_output_fn(char *buf, PRInt32 size, void *stream_closure)
{
  ...
  return sconverter_write_data(msd->streamObj, buf, (PRUint32) size);
}
  ...
  if (msd)
    {
      ...
      // SHERRYHACK
		  // SHERRYHACK!!!  msd->streamObj->abort (msd->streamObj, status);
          
          // SHERRYHACK
          PR_Free (msd->streamObj);
        }
      PR_Free(msd);
    }
}
...
//SHERRYHACK - WORK HERE - we are returning ... should we be!!!
  ...
  // SHERRYHACK - no more!!!?>?????!!
  msd->streamObj = mime_make_output_stream(type, charset, name,
...
{
  NET_StreamClass   *stream = (NET_StreamClass *)newStream;
  if (!stream)
    return;
...
void  *
mime_bridge_create_stream(void *newStreamObj, const char *urlString,
                          int   format_out)
{
  int                     status = 0;
  MimeObject              *obj;
  ...
  */
  ...
  // SHERRY - URL usage for the address!
