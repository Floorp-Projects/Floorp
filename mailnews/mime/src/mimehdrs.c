...
MimeHeaders_build_heads_list(MimeHeaders *hdrs)
{
  ...
  if (hdrs->all_headers_fp + 60 <= hdrs->all_headers_size)
	{
	  char *s = PR_Realloc(hdrs->all_headers, hdrs->all_headers_fp);
  ...
  while (*str)
	{
			   ...
			   *(token_start+parm_len) == '*')
	  {
		  ...
		  if ((*cp == '0' && needUnescape) || (token_end-token_start == parm_len+1))
		  {
			  ...
			  const char *s_quote2 = s_quote1 ? PL_strchr(s_quote1+1, 0x27) : NULL;
...
static char *mime_escape_quotes (char *src)
{
	...
	char *dst = PR_MALLOC((2 * PL_strlen(src)) + 1);
	if (dst)
	{
		char *walkDst = dst;
		while (*src)
...
MimeHeaders_localize_header_name(const char *name, MimeDisplayOptions *opt)
{
  ...
  int display_key =
   (!PL_strcasecmp(name,HEADER_BCC)			 ? MK_MIMEHTML_DISP_BCC :
								   ...
								   PRBool subject_p)
{
  ...
  if (opt->fancy_headers_p)
	{
	  PL_strcpy (out, HEADER_START_JUNK); out += PL_strlen (out);
	  ...
	  PL_strcpy (out, HEADER_END_JUNK); out += PL_strlen (out);
	}
  else
	{
...
#define UNHEX(C) \
  ...
  out = hdrs->obuffer;
  PL_strcpy (out, "</NOBR>"); out += PL_strlen (out);
  if (name) PL_strcpy (out, HEADER_END_JUNK); out += PL_strlen (out);
  *out = 0;
  status = MimeHeaders_write(opt, hdrs->obuffer, out - hdrs->obuffer);
  if (status < 0) goto FAIL;
  else status = 1;
