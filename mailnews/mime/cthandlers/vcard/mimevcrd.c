...
//    status = ((MimeObjectClass*)&MIME_SUPERCLASS)->parse_eof(obj, abort_p);
	...
	v = Parse_MIME(class->vCardString, PL_strlen(class->vCardString));

	if (class->vCardString) {
		PR_Free ((char*) class->vCardString);
...
static int OutputVcardAttribute(MimeObject *obj, VObject *v, const char* id) 
{
	...
	if (prop)
		if (VALUE_TYPE(prop))
		{
			...
			{
				string = PR_MALLOC(PL_strlen((char *) vObjectAnyValue(prop)) + 1);
				if (string)
...
static int OutputBasicVcard(MimeObject *obj, VObject *v)
{
	...
	if (prop)
	{
		if (VALUE_TYPE(prop))
		{
			...
			{
				namestring = PR_MALLOC(PL_strlen((char *) vObjectAnyValue(prop)) + 1);
				if (namestring)
