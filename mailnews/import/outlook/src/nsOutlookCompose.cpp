...
void nsOutlookCompose::GetNthHeader( PRInt32 n, nsString& header, nsString& val, PRBool unwrap)
{
	header.Truncate();
	val.Truncate();
	if (!m_pHeaders)
...
void nsOutlookCompose::GetHeaderValue( const char *pHeader, nsString& val)
{
	val.Truncate();
	if (!m_pHeaders)
		return;
