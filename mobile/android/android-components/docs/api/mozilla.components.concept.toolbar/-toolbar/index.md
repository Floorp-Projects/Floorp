...
| [removeBrowserAction](remove-browser-action.md) | `abstract fun removeBrowserAction(action: `[`Action`](-action/index.md)`): `[`Unit`](https://kotlinlang.org/api/latest/jvm/stdlib/kotlin/-unit/index.html)<br>Removes a previously added action (see [addBrowserAction](add-browser-action.md)). If the the provided actions was never added, this method has no effect. |
