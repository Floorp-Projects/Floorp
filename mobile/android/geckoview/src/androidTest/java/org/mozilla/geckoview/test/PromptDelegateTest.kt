...
class PromptDelegateTest : BaseSessionTest() {
    ...
    @Test fun buttonTest() {
        sessionRule.session.evaluateJS("var res = confirm('Confirm?');")
        ...
        assertThat("Result should match", sessionRule.session.evaluateJS("res") as Boolean,
                equalTo(true))

        sessionRule.session.evaluateJS("var res2 = confirm('Confirm?');")
        ...
        assertThat("Result should match", sessionRule.session.evaluateJS("res2") as Boolean,
                equalTo(false))
    }
    ...
    @Test fun textTest() {
        ...
        // TODO: Investigate. It seems that nsIPrompt::prompt implementation is working correctly...
        /*assertThat("Result should match",
                sessionRule.session.waitForJS("let res = prompt('Prompt:', 'default');") as String,
                equalTo("foo"))*/

        // Remove this when above is fixed.
        sessionRule.session.waitForJS("prompt('Prompt:', 'default');")
    }
