...
public class AuthStateProvider extends ContentProvider {
    ...
    private boolean isTrustedCaller(@NonNull Context context) {
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.P) {
            ...
            callerSignature = packageInfo.signingInfo.getSigningCertificateHistory()[0];

        // For older APIs, we use the deprecated `signatures` field, which isn't aware of certification rotation.
        } else {
            final PackageInfo packageInfo;
            try {
            ...
            callerSignature = packageInfo.signatures[0];
        }
