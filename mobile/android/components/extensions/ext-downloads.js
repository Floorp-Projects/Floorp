...
const DownloadTracker = new (class extends EventEmitter {
  constructor() {
    super();
    ...
    GlobalEventDispatcher.registerListener(this, [DOWNLOAD_CHANGED_MESSAGE]);
  }
  ...
  onEvent(event, data, callback) {
    switch (event) {
      case "GeckoView:WebExtension:DownloadChanged": {
        ...
        if (!downloadItem) {
          return callback.onError("Error: Trying to update unknown download");
        }
