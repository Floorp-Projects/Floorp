...
export class FirefoxChromePool implements ProcessPool {
  ...
  async runTests(files: RunWithFiles[], invalidates?: string[]): Promise<void> {
    console.log(
      "[firefox-pool] Starting Firefox Chrome environment for tests...",
    );
  ...
  private async executeTests(files: RunWithFiles[]): Promise<void> {
    ...
    if (
      this.rpcConnection &&
      this.rpcConnection.readyState === WebSocket.OPEN
    ) {
    ...
    while (attempts < maxAttempts) {
      try {
        const response = await fetch(
          "http://localhost:5181/__firefox_health_check",
        );
        if (response.ok) {
  ...
  private setupHttpPolling(): void {
    ...
    setInterval(async () => {
      try {
        const response = await fetch(
          "http://localhost:5181/__firefox_poll_messages",
        );
        if (response.ok) {
