...
function js_beautify(js_source_text, options={}) {
          ...
          while (esc || input.charAt(parser_pos) !== sep) {
            ...
            if (esc1 && esc1 >= esc2) {
              ...
              if (esc1 && esc1 >= 0x20 && esc1 <= 0x7e) {
                ...
                resulting_string =
                  resulting_string.substr(0, resulting_string.length - esc2 - 2) +
                  (((esc1 === sep) || (esc1 === '\\')) ? '\\' : '') + esc1;
              }
              esc1 = 0;
            }
  ...
  while (js_source_text && (js_source_text.charAt(0) === ' ' ||
         js_source_text.charAt(0) === '\t')) {
  ...
  punct = '+ - * / % & ++ -- = += -= *= /= %= == === != !== > < >= <= >> << ' +
          '>>> >>>= >>= <<= && &= | || ! !! , : ? ^ ^= |= ::';
  punct += ' <%= <% %> <?= <? ?>'; // try not to break markup language identifiers
  punct = punct.split(' ');

  // words which should always start on new line.
  line_starters = ['continue', 'try', 'throw', 'return', 'var', 'if', 'switch',
                   'case', 'default', 'for', 'while', 'break', 'function'];
  ...
  while (true) {
      ...
      if (opt_brace_style=="expand" || opt_brace_style=="expand-strict") {
        ...
        if (opt_brace_style == "expand-strict")
        {
          ...
          if (last_type !== 'TK_OPERATOR') {
            if (last_text === '=' || (is_special_word(last_text) &&
                last_text !== 'else')) {
              print_single_space();
            } else {
              print_newline(true);
    ...
    case 'TK_WORD':
      ...
      if (in_array(token_text.toLowerCase(), ['else', 'catch', 'finally'])) {
        if (last_type !== 'TK_END_BLOCK' || opt_brace_style=="expand" ||
            opt_brace_style=="end-expand" || opt_brace_style == "expand-strict") {
          print_newline();
        } else {
          trim_output(true);
          print_single_space();
      ...
      } else if (is_array(flags.mode) && last_text === ',' &&
                 last_last_text === '}') {
    ...
    case 'TK_STRING':
      ...
      if (last_type === 'TK_END_EXPR' && in_array(flags.previous_mode,
          ['(COND-EXPRESSION)', '(FOR-EXPRESSION)'])) {
        print_single_space();
      } else if (last_type === 'TK_COMMENT' || last_type == 'TK_STRING' ||
                 last_type === 'TK_START_BLOCK' ||
                 last_type === 'TK_END_BLOCK' || last_type === 'TK_SEMICOLON') {
        print_newline();
      } else if (last_type === 'TK_WORD') {
        print_single_space();
      }
    ...
    case 'TK_EQUALS':
      if (flags.var_line) {
        // just got an '=' in a var-line, different formatting/line-breaking,
        // etc will now be done.
    ...
    case 'TK_OPERATOR':
      ...
      if (is_special_word(last_text)) {
        // "return" had a special handling in TK_WORD. Now we need to return the
        // favor.
        print_single_space();
        print_token();
        break;
      ...
      if (token_text == '*' && last_type == 'TK_UNKNOWN' &&
      ...
      if (token_text === ',') {
        ...
        break;
                ...
                (in_array(token_text, ['-', '+']) && (in_array(last_type,
                 ['TK_START_BLOCK', 'TK_START_EXPR', 'TK_EQUALS', 'TK_OPERATOR']) ||
