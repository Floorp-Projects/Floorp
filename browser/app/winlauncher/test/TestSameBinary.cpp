...
inline void
PrintErrorMsg(const char* aMsg)
{
  printf("%s%s\n", kMsgStart, aMsg);
}

inline void
PrintWinError(const char* aMsg)
{
...
template <typename T>
inline void
PrintLauncherError(const mozilla::LauncherResult<T>& aResult,
...
static int
ChildMain(DWORD aExpectedParentPid)
{
  ...
  if (!expectedSameBinary.unwrap()) {
    PrintErrorMsg("IsSameBinaryAsParentProcess returned incorrect result for identical binaries");
    return 1;
  }
  ...
  while (HANDLE p = ::OpenProcess(PROCESS_QUERY_LIMITED_INFORMATION, FALSE,
    ...
    if (curAttempt >= kMaxAttempts) {
      PrintErrorMsg("Exhausted retry attempts waiting for parent pid to become invalid");
      return 1;
    }
  }
  ...
  if (expectedDifferentBinary.isErr()) {
    PrintLauncherError(expectedDifferentBinary, "IsSameBinaryAsParentProcess returned error when we were expecting success");
    return 1;
  }

  if (expectedDifferentBinary.unwrap()) {
    PrintErrorMsg("IsSameBinaryAsParentProcess returned incorrect result for dead parent process");
    return 1;
  }
...
CreateSelfProcess(int argc, wchar_t* argv[])
{
  nsAutoHandle empty;
  ...
  BOOL ok = ::CreateProcessW(argv[0], cmdLine.get(), nullptr, nullptr, FALSE,
...
static int
ParentMain(int argc, wchar_t* argv[])
{
  ...
  if (::WaitForSingleObject(childProc.get(), INFINITE) != WAIT_OBJECT_0) {
    PrintWinError("Unexpected result from WaitForSingleObject on child process");
    return 1;
  }
...
static int
MonitorMain(int argc, wchar_t* argv[])
{
