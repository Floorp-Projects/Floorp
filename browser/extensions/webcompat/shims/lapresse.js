...
/**
 * LaPresse's oauth logins with Google and Facebook redirect through a
...
const STORAGE_ACCESS_ORIGIN = "https://la-presse-prod.firebaseapp.com";

console.warn(
  `When using oauth, Firefox calls the Storage Access API on behalf of the site. See https://bugzilla.mozilla.org/show_bug.cgi?id=1779490 for details.`
);
...
document.documentElement.addEventListener(
  ...
  e => {
    ...
    const isProvider = !!buttonClasses.filter(name =>
      name.startsWith("providerButton")
    ).length;
