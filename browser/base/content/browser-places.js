...
var BookmarkingUI = {
  ...
  toggleBookmarksToolbar(reason) {
    // The PersonalToolbar is always visible on the newtab page,
    // though a user can enable/disable the showing of it on other pages.
...
var AutoShowBookmarksToolbar = {
   ...
   *
   * @param {URI} [options.currentURI] The URI of the selected tab. Must not
   *              be provided if show is provided. Used to calculate if the
   *              toolbar will be shown.
   * @param {Boolean} [options.isNullPrincipal] If the content principal of the
   *              selected tab is the null principal. Used to determine if
   *              about:blank is being used as the new tab page versus a
   *              content-generated load. Required if currentURI is provided.
   * @param {Boolean} [options.show] If the toolbar should be shown or hidden.
   *                  Must not be provided if currentURI is provided.
   * @param {Boolean} [options.animated] If the opening or closing should be animated.
   * @param {Boolean} [options.persist] If the visibility of the toolbar should be persisted.
   * @return {Boolean} False if the toggle was rejected, true otherwise.
   */
  toggleVisibility({ currentURI, isNullPrincipal, show, animated, persist }) {
    ...
    if (toolbar.collapsed != show) {
      // No work needed if the toolbar is already in the state we want.
      return false;
    }
