 ...
 * License.
 *
 * The Original Code is Tabview undo close window test.
 *
...
// Show then immediately hide tabview
...
function test() {
  ...
  // 1) Open a new window for testing undo close after activating tabview
  let newWin = openDialog(location, "", "chrome,all,dialog=no", "about:");
  watchOne(newWin, "load", function() {
    // 2) Toggle tabview twice then close the window
...
function undoClose() {
  // 3) Restore the window and its tab/state
