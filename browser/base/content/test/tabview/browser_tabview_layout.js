...
function testEmptyGroupItem(contentWindow) {
  ...
  for(let t=0; t<numNewTabs; t++) {
    let newItem = contentWindow.gBrowser.loadOneTab("about:blank").tabItem; //, {inBackground: true}).tabItem;
  ...
  // Go to browser view and then back to guarantee that things are created and
  // in the right position. Then call the main test function.
  //
