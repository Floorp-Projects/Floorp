...
class RecentlyClosedTabsList extends HTMLElement {
  ...
  openTabAndUpdate(event) {
    ...
    SessionStore.undoCloseTab(this.getWindow(), index);
  ...
  updateTabsList() {
    let newClosedTabs = SessionStore.getClosedTabData(this.getWindow());
...
class RecentlyClosedTabsContainer extends HTMLElement {
  ...
  connectedCallback() {
    ...
    this.getWindow().gBrowser.tabContainer.addEventListener("TabSelect", this);
  }

  disconnectedCallback() {
    this.getWindow().gBrowser.tabContainer.removeEventListener(
      "TabSelect",
      this
    );
