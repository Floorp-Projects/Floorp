...
add_setup(async function setup_tests() {
  ...
  sandbox
    ...
    .callsFake(
      id =>
        id === SOFT_COLORWAY_THEME_ID || id === NO_INTENSITY_COLORWAY_THEME_ID
    );
  sandbox
  ...
  await createTempTheme(SOFT_COLORWAY_THEME_ID);
  await createTempTheme(NO_INTENSITY_COLORWAY_THEME_ID);
