...
async function mockFxaDeviceConnected(win) {
  ...
  await BrowserTestUtils.waitForEvent(win.gBrowser.tabContainer, "TabOpen");

  is(win.gBrowser.tabs.length, 3, "Tabs strip should contain three tabs");

  win.gURLBar.focus();
  win.gURLBar.value = "https://example.org/pair/auth/complete";
  win.gURLBar.goButton.click();
...
add_task(async function test_close_device_connected_tab() {
  ...
  // remove any extra tabs such as newtab or about:blank to ensure we don't close
  ...
  // user is directed back to the Firefox View tab, the "Device connected" tab has been closed
  // and a newtab tab was opened
  BrowserTestUtils.switchTab(win.gBrowser, fxViewTab);
  is(win.gBrowser.tabs.length, 2, "Tabs strip should only contain two tabs");
