...
class TestReopenFromLibrary(WindowManagerMixin, MarionetteTestCase):
    ...
    def tearDown(self):
        ...
        # self.marionette.set_pref(
        #     "browser.bookmarks.editDialog.showForNewBookmarks",
        #     self.original_showForNewBookmarks_pref,
        # )
        # self.marionette.set_pref(
        #     "browser.tabs.loadBookmarksInTabs", self.original_loadBookmarksInTabs_pref
        # )

        # # Ensure no library windows are left behind, e.g. if the test fails.
    ...
    def test_open_bookmark_from_library_with_no_browser_window_open(self):
        ...
        ).pointer_move(position[0], position[1]).click().click().perform()
