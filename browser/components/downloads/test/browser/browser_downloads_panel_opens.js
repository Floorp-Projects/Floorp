...
add_task(async function checkContextMenu() {
  ...
  contextMenu.activateItem(checkbox);
  is(
  ...
  info("Check context menu for an unrelated button.");
  await openContextMenu(document.getElementById("reload-button"));
  is(checkbox.hidden, true, "Always Open checkbox is hidden.");
  contextMenu.hidePopup();

  info("Open context menu popup directly.");
