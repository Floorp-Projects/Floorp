...
/**
 * Make sure the downloads panel doesn't open if the download was started from
...
add_task(async function test_downloads_panel_after_UCT_dialog() {
  ...
  let publicList = await Downloads.getList(Downloads.PUBLIC);
  // Wait for the dialog button delay just in case.
  ...
  // Make sure our download actually triggers the UCT dialog.
  function alwaysAskForHandlingType(type, ext) {
  ...
  const mimeInfo = alwaysAskForHandlingType("text/plain", "txt");
  ...
  ok(!DownloadsPanel.isPanelShowing, "Panel should not be showing");
