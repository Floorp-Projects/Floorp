...
function fakePref(layout) {
  return ["browser.newtabpage.activity-stream.discoverystream.config", JSON.stringify({
    enabled: true,
...
test_newtab({
    ...
    await pushPrefs(fakePref({
      layout: [{
        ...
        components: [{
          type: "HorizontalRule",
          styles: {
            "@media (min-width: 0)": "content: url(chrome://browser/content)",
            "@media (min-width: 0) *": "content: url(chrome://browser/content)",
            "@media (min-width: 0) { * }": "content: url(chrome://browser/content)",
          },
        }],
  ...
  test: async function test_atrule_filtering() {
    const hr = await ContentTaskUtils.waitForCondition(() => content.document.querySelector("hr"));
    is(content.getComputedStyle(hr).content, "none", "filtered out attempted @media query");
  },
});
