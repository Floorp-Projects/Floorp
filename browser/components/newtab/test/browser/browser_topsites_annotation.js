...
async function assertDatabase({ targetURL, expected }) {
  ...
  let rows = await db.execute("SELECT id FROM moz_places WHERE url = :url", {
    url: targetURL,
  });
  Assert.equal(rows.length, 1);
  const placesId = rows[0].getResultByName("id");

  let expectedTriggeringPlaceId = null;
  if (expected.triggerURL) {
    rows = await db.execute("SELECT id FROM moz_places WHERE url = :url", {
      url: expected.triggerURL,
    });
    Assert.equal(rows.length, 1);
    expectedTriggeringPlaceId = rows[0].getResultByName("id");
  }
  ...
  const triggeringPlaceId = rows[0].getResultByName("triggeringPlaceId");
  Assert.equal(
    triggeringPlaceId,
