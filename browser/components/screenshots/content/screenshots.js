...
class ScreenshotsUI extends HTMLElement {
  ...
  async saveToFile(src) {
    await ScreenshotsUtils.downloadBlob(
      src,
      null,
  ...
  async saveToClipboard(src) {
    let newImg = window.parent.ownerGlobal.gBrowser.selectedBrowser.ownerDocument.createElement(
      "img"
    );

    await ScreenshotsUtils.copyBlob({ blobUrl: src }, newImg);
    this.close();
  }
}
