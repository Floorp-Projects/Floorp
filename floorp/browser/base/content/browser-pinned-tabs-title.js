{
  let apply = () => {
      const showPinnedTabsTitleCSS = document.getElementById("showPinnedTabsTitle-css");
      if (showPinnedTabsTitleCSS) {
        showPinnedTabsTitleCSS.remove();
      }
    
      const enabled = Services.prefs.getBoolPref("floorp.tabs.showPinnedTabsTitle", false);
      if (enabled) {
