...
// Functions that intentionally read beyond the end of an allocation for
// performance The limit is upto 7 bytes past the end of the allocation
...
WASM2C_FUNC_EXPORT void wasm2c_shadow_memory_load(wasm_rt_memory_t* mem,
  ...
  memory_state_iterate(
      mem, ptr, ptr_size, [&](uint32_t index, cell_data_t* data) {
        ...
        // Malloc family of functions store chunk related information in the
        // heap These look like unitialized operations
      ...
      if (data->alloc_state == ALLOC_STATE::UNINIT) {
        ...
        if (index < mem->shadow_memory.heap_base) {
          if (malloc_core_family) {
            ...
            // hard to infer as these access both regular and chunk memory, so
...
WASM2C_FUNC_EXPORT void wasm2c_shadow_memory_store(wasm_rt_memory_t* mem,
  ...
  bool malloc_write_family = is_malloc_family_function(
      func_name, false /* calloc shouldn't modify metadata */, true);
  bool malloc_core_family = is_malloc_family_function(func_name, false, false);
