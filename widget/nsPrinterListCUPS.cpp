...
nsTArray<PrinterInfo> nsPrinterListCUPS::Printers() const {
  ...
  // Another error occurred. printerInfoList may be partially populated, so we
  // could return it without clearing it in the hope that there are some
  // usable dests. However, presuambly CUPS doesn't guarantee that any dests
  // that it added are complete and safe to use when an error occurs?
  FreeDestsAndClear(printerInfoList);
  return {};
}
