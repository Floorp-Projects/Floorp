...
nsMacFinderProgress::~nsMacFinderProgress() { [mProgress release]; }
...
nsMacFinderProgress::Init(const nsAString& path,
  ...
  RefPtr<nsMainThreadPtrHolder<nsIMacFinderProgressCanceledCallback>> cancellationCallbackHandle =
  ...
  mProgress.cancellationHandler = ^{
    NS_DispatchToMainThread(
        NS_NewRunnableFunction("MacFinderProgress::Canceled", [cancellationCallbackHandle] {
          ...
          cancellationCallbackHandle->get()->Canceled();
        }).take());
  };
...
NS_IMETHODIMP
nsMacFinderProgress::UpdateProgress(uint32_t currentProgress, uint32_t totalProgress) {
  NS_OBJC_BEGIN_TRY_ABORT_BLOCK_NSRESULT;
