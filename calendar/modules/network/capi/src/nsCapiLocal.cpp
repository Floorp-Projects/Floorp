...
#ifdef XP_PC
  #define CAPI_LOCAL_NAME "capilocal10.dll"
#elif XP_UNIX
  #define CAPI_LOCAL_NAME "libcapilocal10.so"
#else
  #error MAC not yet supported ... send email to eyork@netscape.com :-)
#endif
...
nsCapiLocal :: nsCapiLocal(nsISupports* outer)
{
  NS_INIT_REFCNT();
  mLibrary = nsnull;
}

nsCapiLocal :: ~nsCapiLocal()
{
  if (nsnull != mLibrary)
    PR_UnloadLibrary(mLibrary);
}

nsresult nsCapiLocal::QueryInterface(REFNSIID aIID, void** aInstancePtr)      
{                                                                        
  if (NULL == aInstancePtr) {                                            
    return NS_ERROR_NULL_POINTER;                                        
  }                                                                      
  static NS_DEFINE_IID(kISupportsIID, NS_ISUPPORTS_IID);
  static NS_DEFINE_IID(kClassIID, kCCapiLocalCID);                  
  if (aIID.Equals(kClassIID)) {                                          
    *aInstancePtr = (void*) (nsCapiLocal *)this;                                        
    AddRef();                                                            
    return NS_OK;                                                        
  }                                                                      
  if (aIID.Equals(kICapiLocalIID)) {                                          
    *aInstancePtr = (void*) (nsICapiLocal *)this;                                        
    AddRef();                                                            
    return NS_OK;                                                        
  }                                                                      
...
NS_IMPL_ADDREF(nsCapiLocal)
NS_IMPL_RELEASE(nsCapiLocal)

nsresult nsCapiLocal :: Init()
{
  ...
  mFunctions.logon_curl                   = (LogonCurl) PR_FindSymbol(mLibrary, "CAPI_LogonCurl");

  return NS_OK;
}


CAPIStatus nsCapiLocal :: CAPI_Capabilities(const char** ppsVal,
                                            const char* psHost,
                                            long lFlags)
{
...
CAPIStatus nsCapiLocal :: CAPI_DeleteEvent( 
...
CAPIStatus nsCapiLocal :: CAPI_DestroyHandles( 
    CAPISession s,              /* i: login session handle  */
...
CAPIStatus nsCapiLocal :: CAPI_DestroyStreams( 
    CAPISession s,              /* i: login session handle  */
...
CAPIStatus nsCapiLocal :: CAPI_FetchEventsByAlarmRange( 
    CAPISession s,              /* i: login session handle  */
...
CAPIStatus nsCapiLocal :: CAPI_FetchEventsByID( 
    CAPISession s,              /* i: login session handle  */
...
CAPIStatus nsCapiLocal :: CAPI_FetchEventsByRange( 
    CAPISession s,              /* i: login session handle  */
    CAPIHandle* pH,             /* i: list of CAPIHandles for fetch  */
    int iHandleCount,           /* i: number of valid handles in ppH  */
    long lFlags,                /* i: bit flags (none at this time; set to 0)  */
...
CAPIStatus nsCapiLocal :: CAPI_GetHandle( 
    CAPISession s,              /* i: login session handle  */
...
CAPIStatus nsCapiLocal :: CAPI_Logoff( 
...
CAPIStatus nsCapiLocal :: CAPI_Logon( 
...
CAPIStatus nsCapiLocal :: CAPI_SetStreamCallbacks ( 
    CAPISession s,
...
CAPIStatus nsCapiLocal :: CAPI_StoreEvent( 
    CAPISession s,              /* i: login session handle  */
...
CAPIStatus nsCapiLocal :: CAPI_LogonCurl( 
    const char* psCurl,         /* i: Calendar store (and ":extra" information )  */
    const char* psPassword,     /* i: password for sUser  */
    long lFlags,                /* i: bit flags (none at this time; set to 0)  */
    CAPISession* pSession)     /* o: the session  */
{
    return (mFunctions.logon_curl(psCurl,psPassword,lFlags,pSession));
}
