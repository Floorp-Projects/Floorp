...
nsresult MRJPlugin::StartupJVM()
{
    if (mSession == NULL) {
        ...
        // Compute location of ~/Library/Logs/JavaConsole.log
...
/**
 * Poor Man's Mac OS X Java Console implementation. MRJSession::open()
 * opens ~/Library/Logs/JavaConsole.log, and redirects all output
 * from System.out and System.err to this file. Rather than
 * implementing a Java based console, just take advantage of
 * Mac OS X's /Applications/Utilities/Console.app capability
 * of showing log files. The following simply uses Launch Services
 * to open whatever application normally opens ".log" files.
 * A more sophisticated implementation would launch Console.app
 * with the log file, but this works just fine for now.
 */
...
NS_METHOD MRJPlugin::Print(const char* msg, const char* /* encodingName */)
{
