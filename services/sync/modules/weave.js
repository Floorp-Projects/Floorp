...
BookmarksEngine.prototype = {
  ...
  _getServerData: function BmkEngine__getServerData(onComplete) {
	...
	let snap = new SnapshotStore();
        ...
        if (this._snapshot.version < status.snapVersion) {
          ...
          snap.data = Weave.Crypto.PBEdecrypt(resp.responseText,
					      this._cryptoId,
          ...
          allDeltas = Weave.Crypto.PBEdecrypt(resp.responseText,
					      this._cryptoId,
      ...
      case 404:
        ...
        ret.snapEncryption = Weave.Crypto.defaultAlgorithm;
        ret.deltasEncryption = Weave.Crypto.defaultAlgorithm;
  ...
  _fullUpload: function BmkEngine__fullUpload(onComplete) {
    ...
    try {
      let data = Weave.Crypto.PBEencrypt(this._snapshot.serialize(),
					 this._cryptoId);
      ...
      this._dav.PUT("bookmarks-status.json",
                    uneval({GUID: this._snapshot.GUID,
                            ...
                            snapEncryption: Weave.Crypto.defaultAlgorithm,
  ...
  _sync: function BmkEngine__sync(onComplete) {
	...
	localJson.applyCommands(clientChanges);
	  ...
	  let data = Weave.Crypto.PBEencrypt(serializeCommands(server.deltas),
					     this._cryptoId);
          ...
          this._dav.PUT("bookmarks-status.json",
                        uneval({GUID: this._snapshot.GUID,
                                ...
                                deltasEncryption: Weave.Crypto.defaultAlgorithm,
                                Bookmarkscount: c}), cont);
          let statusPut = yield;
...
let Weave = {};
Weave.Crypto = new Crypto();
Weave.Service = new BookmarksSyncService();
