...
#define UNICHAR_IN_RANGE(a, b, c) \
 (((uint16_t)(a) <= (uint16_t)(b))&&((uint16_t)(b) <= (uint16_t)(c)))

static const uint16_t gGBKToUnicodeTable[MAX_GBK_LENGTH] = {
#include "cp936map.h"
};
...
int main()
{
    ...
    for (i = 0; i < 0xA000 - 0x4e00; i += 8) {
        printf("0x%.4x, ", gUnicodeToGBKTable[i + 0]);
        printf("0x%.4x, ", gUnicodeToGBKTable[i + 1]);
        printf("0x%.4x, ", gUnicodeToGBKTable[i + 2]);
        printf("0x%.4x, ", gUnicodeToGBKTable[i + 3]);
        printf("0x%.4x, ", gUnicodeToGBKTable[i + 4]);
        printf("0x%.4x, ", gUnicodeToGBKTable[i + 5]);
        printf("0x%.4x, ", gUnicodeToGBKTable[i + 6]);
        printf("0x%.4x,\n", gUnicodeToGBKTable[i + 7]);
    }
