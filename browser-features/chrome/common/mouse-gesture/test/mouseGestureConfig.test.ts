...
import type { GestureDirection, GesturePattern, MouseGestureConfig } from "../config.ts";
...
function testPatternToStringEmpty(): void {
  assertEquals(patternToString([]), "", "empty pattern should return empty string");
}
...
function testRoundTripSingleDirection(): void {
  ...
  assertEquals(stringToPattern(patternToString(original))[0], "up", "single round-trip");
}
...
function testGestureDirectionCodecValidDirections(): void {
  ...
  for (const dir of validDirections) {
    ...
    assert(result._tag === "Right", `${dir} should be a valid GestureDirection`);
  }
}
...
function testGestureDirectionCodecRejectsNumber(): void {
  ...
  assert(result._tag === "Left", "number should fail GestureDirection validation");
}

function testGestureDirectionCodecRejectsNull(): void {
  const result = GestureDirectionCodec.decode(null);
  assert(result._tag === "Left", "null should fail GestureDirection validation");
}
...
function testGestureDirectionCodecRejectsCamelCaseVariant(): void {
  ...
  assert(result._tag === "Left", "up-right (hyphenated) should fail validation");
}
...
function testGestureActionCodecEmptyPattern(): void {
  ...
  assert(result._tag === "Right", "empty pattern array should pass validation (no min length)");
}
...
function testDefaultConfigStructure(): void {
  ...
  assert(typeof config.rockerGesturesEnabled === "boolean", "rockerGesturesEnabled should be boolean");
  assert(typeof config.wheelGesturesEnabled === "boolean", "wheelGesturesEnabled should be boolean");
  assert(typeof config.sensitivity === "number", "sensitivity should be number");
...
function testDefaultConfigContextMenu(): void {
  ...
  assert(typeof cm.preventionTimeout === "number", "preventionTimeout should be number");
  assert(cm.minDistance >= 5, `minDistance should be >= 5, got ${cm.minDistance}`);
}

function testDefaultConfigActionsNonEmpty(): void {
  assert(defaultConfig.actions.length > 0, "default actions should not be empty");
}
...
function testDefaultConfigKnownActions(): void {
  ...
  assert(actionNames.includes("gecko-close-tab"), "should include gecko-close-tab");
  assert(actionNames.includes("gecko-open-new-tab"), "should include gecko-open-new-tab");
}
...
function testDefaultConfigActionPatterns(): void {
  ...
  assertEquals(backAction.pattern[0], "left", "gecko-back pattern should be [left]");

  const forwardAction = config.actions.find((a) => a.action === "gecko-forward");
  assert(forwardAction !== undefined, "gecko-forward should exist");
  assertEquals(forwardAction.pattern[0], "right", "gecko-forward pattern should be [right]");

  const reloadAction = config.actions.find((a) => a.action === "gecko-reload");
  assert(reloadAction !== undefined, "gecko-reload should exist");
  assertEquals(reloadAction.pattern.length, 2, "gecko-reload should have 2-direction pattern");
  assertEquals(reloadAction.pattern[0], "up", "gecko-reload first direction");
  assertEquals(reloadAction.pattern[1], "down", "gecko-reload second direction");
}
...
function testDefaultConfigTrailColor(): void {
  ...
  assert(color.length === 7, `trailColor should be 7 chars (#RRGGBB), got ${color.length}`);
}
...
function testDefaultConfigNineActions(): void {
  assertEquals(defaultConfig.actions.length, 9, "default config should have exactly 9 actions");
}

function testDefaultConfigActionPatternsUnique(): void {
  const patternStrings = defaultConfig.actions.map((a) => patternToString(a.pattern));
  const uniquePatterns = new Set(patternStrings);
  assertEquals(uniquePatterns.size, patternStrings.length, "all action patterns should be unique");
}
...
function testDefaultConfigEnabledFalse(): void {
  ...
  assertEquals(defaultConfig.enabled, false, "default config enabled should be false");
}

function testDefaultConfigHasAllBaseFields(): void {
  const config = defaultConfig;
  assert(config.enabled !== undefined, "enabled should exist");
  assert(config.rockerGesturesEnabled !== undefined, "rockerGesturesEnabled should exist");
  assert(config.wheelGesturesEnabled !== undefined, "wheelGesturesEnabled should exist");
...
function testConfigCodecValidWithOptionalEnabled(): void {
  ...
  const result = MouseGestureConfigCodec.decode(validConfig);
  assert(result._tag === "Right", "valid config with optional enabled should pass");
}
...
function testConfigCodecInvalidMissingFields(): void {
  ...
  assert(result._tag === "Left", "missing required fields should fail validation");
}
...
function testConfigCodecInvalidSensitivityType(): void {
  ...
  const result = MouseGestureConfigCodec.decode(invalidConfig);
  assert(result._tag === "Left", "wrong sensitivity type should fail validation");
}
...
function testConfigCodecInvalidActionPattern(): void {
  ...
  const result = MouseGestureConfigCodec.decode(invalidConfig);
  assert(result._tag === "Left", "invalid direction in pattern should fail validation");
}
...
function testConfigCodecInvalidTrailWidthType(): void {
  ...
  const result = MouseGestureConfigCodec.decode(invalidConfig);
  assert(result._tag === "Left", "wrong trailWidth type should fail validation");
}
...
function testConfigCodecInvalidContextMenuStructure(): void {
  ...
  const result = MouseGestureConfigCodec.decode(invalidConfig);
  assert(result._tag === "Left", "invalid contextMenu structure should fail validation");
}
...
function testConfigCodecInvalidActionMissingAction(): void {
  ...
  const result = MouseGestureConfigCodec.decode(invalidConfig);
  assert(result._tag === "Left", "action missing 'action' field should fail validation");
}
...
function testPrefConstants(): void {
  assertEquals(MOUSE_GESTURE_ENABLED_PREF, "floorp.mousegesture.enabled", "enabled pref");
  assertEquals(MOUSE_GESTURE_CONFIG_PREF, "floorp.mousegesture.config", "config pref");
}
...
function testStrDefaultConfigRoundtrip(): void {
  const parsed = JSON.parse(strDefaultConfig);
  assertEquals(parsed.rockerGesturesEnabled, defaultConfig.rockerGesturesEnabled, "roundtrip rockerGesturesEnabled");
  assertEquals(parsed.wheelGesturesEnabled, defaultConfig.wheelGesturesEnabled, "roundtrip wheelGesturesEnabled");
  assertEquals(parsed.sensitivity, defaultConfig.sensitivity, "roundtrip sensitivity");
  assertEquals(parsed.showTrail, defaultConfig.showTrail, "roundtrip showTrail");
  assertEquals(parsed.showLabel, defaultConfig.showLabel, "roundtrip showLabel");
  assertEquals(parsed.trailColor, defaultConfig.trailColor, "roundtrip trailColor");
  assertEquals(parsed.trailWidth, defaultConfig.trailWidth, "roundtrip trailWidth");
  assertEquals(parsed.actions.length, defaultConfig.actions.length, "roundtrip actions count");
}
...
function testStrDefaultConfigActionsMatch(): void {
  ...
  for (let i = 0; i < defaultConfig.actions.length; i++) {
    ...
    assertEquals(serialized.pattern.length, original.pattern.length, `action ${i} pattern length`);
    for (let j = 0; j < original.pattern.length; j++) {
      assertEquals(serialized.pattern[j], original.pattern[j], `action ${i} pattern[${j}]`);
    }
  }
}
...
const tests: TestCase[] = [
  ...
  { name: "patternToString multiple directions", fn: testPatternToStringMultiple },
  ...
  { name: "patternToString diagonal directions", fn: testPatternToStringDiagonal },
  { name: "patternToString five-direction pattern", fn: testPatternToStringLongPattern },
  // stringToPattern
  { name: "stringToPattern single direction", fn: testStringToPatternSingle },
  { name: "stringToPattern multiple directions", fn: testStringToPatternMultiple },
  ...
  // GestureDirectionCodec
  { name: "GestureDirectionCodec valid directions", fn: testGestureDirectionCodecValidDirections },
  { name: "GestureDirectionCodec invalid direction", fn: testGestureDirectionCodecInvalidDirection },
  { name: "GestureDirectionCodec rejects number", fn: testGestureDirectionCodecRejectsNumber },
  { name: "GestureDirectionCodec rejects null", fn: testGestureDirectionCodecRejectsNull },
  { name: "GestureDirectionCodec rejects uppercase", fn: testGestureDirectionCodecRejectsUppercase },
  { name: "GestureDirectionCodec rejects hyphenated", fn: testGestureDirectionCodecRejectsCamelCaseVariant },
  // GestureActionCodec
  { name: "GestureActionCodec valid", fn: testGestureActionCodecValid },
  { name: "GestureActionCodec valid diagonal pattern", fn: testGestureActionCodecValidDiagonalPattern },
  { name: "GestureActionCodec invalid pattern type", fn: testGestureActionCodecInvalidPatternType },
  { name: "GestureActionCodec invalid action type", fn: testGestureActionCodecInvalidActionType },
  { name: "GestureActionCodec missing pattern", fn: testGestureActionCodecMissingPattern },
  { name: "GestureActionCodec missing action", fn: testGestureActionCodecMissingAction },
  { name: "GestureActionCodec empty pattern", fn: testGestureActionCodecEmptyPattern },
  { name: "GestureActionCodec multiple directions", fn: testGestureActionCodecMultipleDirections },
  // defaultConfig structure
  { name: "defaultConfig structure", fn: testDefaultConfigStructure },
  { name: "defaultConfig sensitivity range", fn: testDefaultConfigSensitivityRange },
  { name: "defaultConfig contextMenu", fn: testDefaultConfigContextMenu },
  { name: "defaultConfig actions non-empty", fn: testDefaultConfigActionsNonEmpty },
  { name: "defaultConfig actions structure", fn: testDefaultConfigActionsStructure },
  { name: "defaultConfig known actions", fn: testDefaultConfigKnownActions },
  { name: "defaultConfig action patterns", fn: testDefaultConfigActionPatterns },
  ...
  { name: "defaultConfig action patterns unique", fn: testDefaultConfigActionPatternsUnique },
  // defaultConfig implied BASE fields
  { name: "defaultConfig enabled false", fn: testDefaultConfigEnabledFalse },
  { name: "defaultConfig has all base fields", fn: testDefaultConfigHasAllBaseFields },
  // MouseGestureConfigCodec
  { name: "ConfigCodec valid minimal", fn: testConfigCodecValidMinimal },
  { name: "ConfigCodec valid with optional enabled", fn: testConfigCodecValidWithOptionalEnabled },
  { name: "ConfigCodec valid empty actions", fn: testConfigCodecValidEmptyActions },
  { name: "ConfigCodec invalid missing fields", fn: testConfigCodecInvalidMissingFields },
  { name: "ConfigCodec invalid sensitivity type", fn: testConfigCodecInvalidSensitivityType },
  { name: "ConfigCodec invalid action pattern", fn: testConfigCodecInvalidActionPattern },
  { name: "ConfigCodec invalid trailWidth type", fn: testConfigCodecInvalidTrailWidthType },
  { name: "ConfigCodec invalid showTrail type", fn: testConfigCodecInvalidShowTrailType },
  { name: "ConfigCodec invalid contextMenu structure", fn: testConfigCodecInvalidContextMenuStructure },
  { name: "ConfigCodec invalid action missing action field", fn: testConfigCodecInvalidActionMissingAction },
  // pref constants
  { name: "pref constants", fn: testPrefConstants },
  { name: "strDefaultConfig is valid JSON", fn: testStrDefaultConfigIsJson },
  { name: "strDefaultConfig roundtrip values", fn: testStrDefaultConfigRoundtrip },
  { name: "strDefaultConfig actions match", fn: testStrDefaultConfigActionsMatch },
];
