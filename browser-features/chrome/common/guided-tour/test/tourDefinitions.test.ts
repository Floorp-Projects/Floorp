...
function testRequestPrefStartsWorkspacesTour(): void {
  const controller = new TourController();
  Services.prefs.setStringPref(
    TOUR_REQUEST_PREF,
    JSON.stringify({ tourId: "workspaces" }),
  );
  try {
    assert(
      controller.tour()?.id === "workspaces",
      "writing the request pref must start the workspaces tour",
    );
    assert(
      controller.status() === "active",
      "the tour controller should be active after the request pref is consumed",
    );
  } finally {
...
export async function runAllTests(): Promise<void> {
  const tests: TestCase[] = [
    ...
    {
      name: "request pref starts the workspaces tour",
      fn: testRequestPrefStartsWorkspacesTour,
    },
  ];
