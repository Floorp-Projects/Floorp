...
import { STATIC_PANEL_DATA } from "../../common/panel-sidebar/data/static-panels.ts";
...
function testAllPanelsHaveRequiredFields(): void {
  for (const [key, panel] of Object.entries(STATIC_PANEL_DATA)) {
    assert(typeof panel.url === "string" && panel.url.length > 0, `${key}: url should be non-empty string`);
    assert(typeof panel.icon === "string" && panel.icon.length > 0, `${key}: icon should be non-empty string`);
    assert(typeof panel.l10n === "string" && panel.l10n.length > 0, `${key}: l10n should be non-empty string`);
    assert(typeof panel.defaultWidth === "number" && panel.defaultWidth > 0, `${key}: defaultWidth should be positive number`);
  }
}
...
export async function runAllTests(): Promise<void> {
  const tests: TestCase[] = [
    ...
    { name: "all panels have required fields", fn: testAllPanelsHaveRequiredFields },
    { name: "panel keys start with floorp//", fn: testPanelKeysStartWithFloorp },
  ];
