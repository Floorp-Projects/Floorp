...
} from "../../common/tabbar/multirow-tabbar/dom-utils.ts";
...
function testFindChildIndexEmpty(): void {
  ...
  const child = document.createElement("span");
  assertEquals(
    findChildIndex(container, child),
    -1,
    "empty container → -1",
  );
}
...
function testGetTabsToolbar(): void {
  ...
  assert(
    toolbar !== null,
    "TabsToolbar should exist in browser chrome",
  );
...
function testResolveTabsContainer(): void {
  ...
  assert(
    container !== null,
    "tabs container should exist in browser chrome",
  );
}
...
export async function runAllTests(): Promise<void> {
  ...
  if (failures.length > 0) {
    throw new Error(
      `tabbarDomUtils.test.ts failures: ${failures.join(" | ")}`,
    );
  }
}
