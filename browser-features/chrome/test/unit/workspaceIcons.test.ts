...
export async function runAllTests(): Promise<void> {
  const tests: TestCase[] = [
    ...
    { name: "getWorkspaceIconUrl undefined", fn: testGetWorkspaceIconUrlUndefined },
    { name: "getWorkspaceIconUrl invalid", fn: testGetWorkspaceIconUrlInvalid },
    { name: "all icons resolve to data URLs", fn: testAllIconsResolveToDataUrls },
  ];
  ...
  if (failures.length > 0) {
    throw new Error(
      `workspaceIcons.test.ts failures: ${failures.join(" | ")}`,
    );
  }
}
