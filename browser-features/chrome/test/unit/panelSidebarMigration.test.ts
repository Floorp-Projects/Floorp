...
import { convertSidebar } from "../../common/panel-sidebar/data/migration.ts";
...
function testEmptySidebar(): void {
  ...
  assertEquals(result.data.length, 0, "empty sidebar should produce empty data");
}
...
function testZoomLevelPreserved(): void {
  ...
  assertEquals(
    result.data[0].zoomLevel,
    1.5,
    "zoomLevel should be preserved",
  );
}
