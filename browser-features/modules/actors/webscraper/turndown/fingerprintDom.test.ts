...
function testFingerprintIgnoresVolatileAttributes(): void {
  withFixture((root) => {
    ...
    assertEquals(second.short, first.short, "id/class/style/data-* changes should not alter short fingerprint");
    assertEquals(second.full, first.full, "id/class/style/data-* changes should not alter full fingerprint");
  });
}
...
function testFingerprintDeterminismWithNoisyChildren(): void {
  withFixture((root) => {
    ...
    assertEquals(withNoiseSecond.short, withNoiseFirst.short, "fingerprint should be deterministic on repeated calls");
    assertEquals(withNoiseSecond.full, withNoiseFirst.full, "full fingerprint should be deterministic on repeated calls");

    const noisyNodes = Array.from(card.querySelectorAll("noscript,style")) as Element[];
    for (const noisyNode of noisyNodes) {
      noisyNode.remove();
    }

    const withoutNoise = generateFingerprint(card);
    assertEquals(typeof withoutNoise.short, "string", "short fingerprint should remain available after DOM mutation");
    assertEquals(typeof withoutNoise.full, "string", "full fingerprint should remain available after DOM mutation");
    assertEquals(withoutNoise.short.length, 8, "short fingerprint should keep expected length");
    assertEquals(withoutNoise.full.length, 16, "full fingerprint should keep expected length");
  });
}
...
function testFingerprintTextCacheConsistency(): void {
  withFixture((root) => {
    ...
    for (const element of elements) {
      ...
      assertEquals(withCache.short, noCache.short, "text cache should not change short fingerprint output");
      assertEquals(withCache.full, noCache.full, "text cache should not change full fingerprint output");
    }
  });
}
...
function testFindAndResolveByFingerprint(): void {
  withFixture((root) => {
    ...
    assertEquals(shortFound, target, "short fingerprint should resolve matching element");

    const fullFound = findElementByFingerprint(body, targetFp.full);
    assertEquals(fullFound, target, "full fingerprint should resolve matching element");

    const allFound = findElementsByFingerprint(body, targetFp.full);
    assertEquals(allFound.length, 1, "findElementsByFingerprint should return one match for unique fingerprint");
    assertEquals(allFound[0], target, "findElementsByFingerprint should contain the target element");

    const invalidLookup = findElementByFingerprint(body, "bad-fp");
    assertEquals(invalidLookup, null, "invalid fingerprint format should return null without traversal");

    const invalidList = findElementsByFingerprint(body, "bad-fp");
    assertEquals(invalidList.length, 0, "invalid fingerprint format should return empty match list");

    const overlayFound = findElementByFingerprint(body, overlayFp.short);
    assertEquals(overlayFound, null, "highlight overlay nodes should be excluded from fingerprint lookup traversal");

    const selectorPreferred = resolveElementLocator(doc, {
      selector: "#target-button",
      fingerprint: targetFp.short,
    });
    assertEquals(selectorPreferred, target, "CSS selector should take priority over fingerprint when both are provided");

    const fingerprintFallback = resolveElementLocator(doc, { fingerprint: targetFp.short });
    assertEquals(fingerprintFallback, target, "fingerprint locator should resolve element when selector is absent");
  });
}

export function runAllTests(): void {
  testFingerprintIgnoresVolatileAttributes();
  testFingerprintDeterminismWithNoisyChildren();
  testFingerprintTextCacheConsistency();
  testFindAndResolveByFingerprint();
}
