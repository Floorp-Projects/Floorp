...
{
    JSClass* c = dynamic_cast<JSClass*>(t);
    return (c && c->hasMethod(name, slotIndex));
}


static bool isStaticName(JSClass *c, const StringAtom &name, JSType*& type, bool &isConstructor)
{
    do {
        if (c->hasStatic(name, type, isConstructor))
            return true;
        c = c->getSuperClass();
    } while (c);
    return false;
