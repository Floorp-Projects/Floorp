...
// least significant bit will be set unless y was originally zero.
    ...
    uint32 *BigInt::freeLists[maxLgGrossSize+1];


// Allocate a BigInt with 2^lgGrossSize words.  The BigInt must not currently
// contain a number.
    void BigInt::allocate(uint lgGrossSize)
    {
        ...
        if (w) {
            ...
            FREE_DTOA_LOCK(0);
            w = static_cast<uint32 *>(STD::malloc(max(uint32(grossSize *
                                                             sizeof(uint32)),
...
// Copy b into this BigInt, which must be uninitialized.
    void BigInt::initCopy(const BigInt &b)
    {
...
// fit in the new lgGrossSize.
    void BigInt::setLgGrossSize(uint lgGrossSize)
    {
        ASSERT(words);
...
// of significant bits in b in bits.  d must be finite and nonzero.
    void BigInt::init(double d, int32 &e, int32 &bits)
    {
        allocate(1);
...
// Let this = this*m + a.  Both a and m must be between 0 and 65535 inclusive.
    void BigInt::mulAdd(uint m, uint a)
    {
#ifdef NATIVE_INT64
        uint64 carry, y;
#else
...
// Initialize this BigInt to m-n.  This BigInt must not have a previous value.
    void BigInt::initDiff(const BigInt &m, const BigInt &n)
    {
...
// start with a 1 bit, and the returned quotient must be less than 36.
    int32 BigInt::quoRem(const BigInt &S)
    {
#ifdef NATIVE_INT64
...
// nonnegative. divisor must be between 1 and 65536.
    uint32 BigInt::divRem(uint32 divisor)
    {
        uint32 n = size;
        uint32 remainder = 0;
        ...
        // Decrease the size of the number if its most significant word is now
        // zero.
        if (bx[n-1] == 0)
            size--;
        return remainder;
    ...
    double BigInt::b2d(int32 &e) const
    {
        double d;
    ...
    double BigInt::ratio(const BigInt &denominator) const
    {
        int32 ka, kb;
    ...
    void BigInt::s2b(const char *s, int32 nd0, int32 nd, uint32 y9)
    {
        int32 i;
        int32 x, y;
      ...
      dig_done:
        ...
        if (nd <= DBL_DIG && FLT_ROUNDS == 1) {
            ...
            if (e > 0) {
                ...
                i = DBL_DIG - nd;
                if (e <= Ten_pmax + i) {
                    // A fancier test would sometimes let us do this for larger
                    // i values.
                    e -= i;
                    rv *= tens[i];
                    rv *= tens[e];
        ...
        if (e1 > 0) {
            ...
            if (e1 &= ~15) {
                ...
                for (j = 0; e1 > 0; j++, e1 >>= 1)
                    if (e1 & 1)
                        rv *= tinytens[j];
                if (scale && (j = P + 1 - int32((word0(rv) & Exp_mask) >>
                                                Exp_shift)) > 0) {
        ...
        // Put digits into bd: true value = bd * 10^e
        {
            ...
            for (;;) {
                ...
                i = delta.cmp(bs);
                if (i < 0) {
                    // Error is less than half an ulp -- check for special case
                    // of mantissa a power of two.
...
// null-terminated. Leading Unicode whitespace is skipped.
    ...
    public:
        BinaryDigitReader(uint base, const char16 *digitsBegin,
                          const char16 *digitsEnd) :
                base(base), digitMask(0), digits(digitsBegin),
                digitsEnd(digitsEnd) {}
                           ...
                           const char16 *&numEnd, uint base)
    {
        ...
        if (value >= 9007199254740992.0) {
            if (base == 10) {
                ...
                // may be inaccurate.  Call stringToDouble to get the correct
                // answer.
                const char16 *numEnd2;
                value = stringToDouble(start, str1, numEnd2);
                ASSERT(numEnd2 == str1);

            } else if (base == 2 || base == 4 || base == 8 || base == 16 ||
                       base == 32) {
                // The number may also be inaccurate for one of these bases.
                // This happens if the addition in value*base + digit causes a
                // round-down to an even least significant mantissa bit when the
                // first dropped bit is a one.  If any of the following digits
                // in the number (which haven't been added in yet) are nonzero
                // then the correct action would have been to round up instead
                // of down.  An example of this occurs when reading the number
                ...
                if (bit == 1) {
                    ...
                    if (bit2 >= 0) {
                        double factor = 2.0;
                        // sticky is 1 if any bit beyond the 54th is 1
                        int sticky = 0;
                        int bit3;
...
// or dtosVariableBufferSize, whichever is appropriate.
    char *doubleToStr(char *buffer, size_t DEBUG_ONLY(bufferSize), double value, DToStrMode mode, int precision)
    {
...
// precision should not exceed 101.
    void appendDouble(String &dst, double value, DToStrMode mode, int precision)
    {
...
// precision should not exceed 101.
    void printDouble(Formatter &f, double value, DToStrMode mode, int precision)
    {
        char buffer[dtosVariableBufferSize(101)];
        ASSERT(uint(precision) <= 101);
