...
#undef DEBUG_EARLY_BINDING
	...
	{		
		...
		for (const byte* pc = code_pos, *code_end=code_pos+code_length; pc < code_end; pc += size)
		{
			...
			if ( blockStates && (blockState = blockStates->get((int)pc)) != 0 )
			{
				...
				for (int i=0, n=info->exceptions->exception_count; i < n; i++)
				{
					...
					if (pc >= code_pos + handler->from && pc <= code_pos + handler->to)
					{
						...
						if (opCanThrow[opcode] || pc == code_pos + handler->from)
						{
							...
							{
								FrameState *catchState = blockStates->get((int)target);
			...
			case OP_debugfile:
			{
				...
				if (mir) mir->emit(state, opcode, (int)AvmCore::atomToString(filename));
				#endif
				break;
			}
			...
			case OP_dxns:
			{
				...
				if (mir) mir->emit(state, opcode, (int)AvmCore::atomToString(uri));
				break;
			}
			...
			case OP_pushstring: 
			{
				...
				if (index > 0 && index < pool->constantStringCount)
				{
					...
					if (mir)mir->emitIntConst(state, sp+1, (int)value);
			...
			case OP_newclass: 
			{
				...
				if (mir)
				{
					...
					mir->emit(state, opcode, (int)(void*)pool->cinits[imm30], sp, ctraits);
				}
			...
			case OP_finddef: 
			{
				...
				if (script != (AbstractFunction*)BIND_NONE && script != (AbstractFunction*)BIND_AMBIGUOUS)
				{
					...
					if (mir) mir->emit(state, opcode, (int)&multiname, sp+1, script->declaringTraits);
				...
				else
				{
					...
					// a runtime exception
					if (mir) mir->emit(state, opcode, (int)&multiname, sp+1, OBJECT_TYPE);
			...
			case OP_initproperty:
			{
				...
				if (mir)
				{
					...
					mir->emit(state, opcode, (int)&multiname);
				}
				state->pop(n);
			...
			case OP_getdescendants:
			{
				...
				if (mir)
				{
					...
					mir->emit(state, opcode, (int)&multiname, 0, NULL);
				}
			...
			case OP_deleteproperty:
			{
				...
				{
					emitCheckNull(sp-(n-1));
					mir->emit(state, opcode, (int)&multiname, 0, BOOLEAN_TYPE);
				}
			...
			case OP_astype:
			{
				...
				if (!canAssign(t, rhs) || !Traits::isMachineCompatible(t, rhs))
				{
					...
					if (mir)
						mir->emit(state, OP_astype, (int)t, index, resultType);
					state->pop_push(1, t);
				}
			...
			case OP_istype: 
			{
				...
				if (mir) mir->emit(state, opcode, (int)itraits, sp, BOOLEAN_TYPE);
				state->pop();
				state->pop();
			...
			case OP_callpropvoid:
			{
				...
				{
					mir->emitSetContext(state, NULL);
					mir->emit(state, opcode, (int)&multiname, argc, NULL);
				}
				state->pop_push(n, NULL);
				if (opcode == OP_callpropvoid)
					state->pop();
			...
			case OP_getsuper:
			{
				...
				{
					mir->emitSetContext(state, NULL);
					mir->emit(state, opcode, (int)&multiname, 0, propType);
				}
				state->pop_push(n, propType);
				break;
			}
	...
	void Verifier::emitFindProperty(AbcOpcode opcode, Multiname& multiname)
	{
				...
				if (!early && index <= 0)
				{
					...
					if (script != (AbstractFunction*)BIND_NONE && script != (AbstractFunction*)BIND_AMBIGUOUS)
					{
						if (mir)
						{
							...
							{
								// found a single matching traits
								mir->emit(state, OP_finddef, (int)&multiname, state->sp()+1, script->declaringTraits);
							}
						}
						state->push(script->declaringTraits, true);
						early = true;
		...
		if (!early)
		{
			uint32 n=1;
			checkPropertyMultiname(n, multiname);
			if (mir) mir->emit(state, opcode, (int)&multiname, 0, OBJECT_TYPE);
	...
	void Verifier::emitGetProperty(Multiname &multiname, int n)
	{
		...
		{
			mir->emitSetContext(state, NULL);
			mir->emit(state, OP_getproperty, (int)&multiname, 0, propType);
		}
		state->pop_push(n, propType);
	}
	...
	FrameState *Verifier::getFrameState(int targetpc)
	{
		...
		if ( (targetState = blockStates->get((int)target)) == 0 ) 
		{
			targetState = newFrameState();
			targetState->pc = target - code_pos;
			blockStates->put((int)target, targetState);
			labelCount++;
		}
		return targetState;
	}
