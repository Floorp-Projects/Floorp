...
{
    return IsSlotUndemotable(cx, f, slot, cx->regs->pc);
}
...
{
    JS_ASSERT(cx->regs->sp + 2 <= cx->fp->slots() + cx->fp->script->nslots);
...
{
    stack(0, lir->insImmf(GET_UINT16(cx->regs->pc)));
    return ARECORD_CONTINUE;
}
...
{
    stack(0, lir->insImmf(GET_UINT24(cx->regs->pc)));
    return ARECORD_CONTINUE;
}
...
{
    JSObject *obj = GetBuiltinFunction(cx, GET_INDEX(cx->regs->pc));
    if (!obj)
...
{
    stack(0, lir->insImmf(GET_INT8(cx->regs->pc)));
    return ARECORD_CONTINUE;
}
...
{
    stack(0, lir->insImmf(GET_INT32(cx->regs->pc)));
    return ARECORD_CONTINUE;
}
