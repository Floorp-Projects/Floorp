...
ParseHexFloatLiteral(const char16_t* cur, const char16_t* end, Float* result)
{
    ...
    for (; cur != end; cur++) {
        ...
        if (!sawFirstNonZero) {
            if (digit == 0)
                continue;
            // Determine the initial exponent; if we're after the dot, count the
            // number of zeros from the dot to here. Set up significandPos to
            // put the first nonzero at the most significant bit.
            int_fast8_t lz = CountLeadingZeroes4(digit);
            exponent = (!dot ? 1 : ((dot + 1) - cur)) * 4 - lz - 1;
