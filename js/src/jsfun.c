...
call_enumerate(JSContext *cx, JSObject *obj)
{
    ...
    names = js_GetLocalNameArray(cx, fun);
    if (!names)
        return JS_FALSE;
  ...
  out:
    js_PutLocalNameArray(cx, fun, names);
    return ok;
}
...
js_GetLocalNameArray(JSContext *cx, JSFunction *fun)
{
     ...
     */
    names = JS_malloc(cx, (size_t) n * sizeof *names);
    if (!names)
        return NULL;
...
XDRLocalNames(JSXDRState *xdr, JSFunction *fun)
{
...
#endif
    if (xdr->mode == JSXDR_ENCODE) {
        names = js_GetLocalNameArray(xdr->cx, fun);
        if (!names)
            return JS_FALSE;
    }
  ...
  out:
    if (xdr->mode == JSXDR_ENCODE)
        js_PutLocalNameArray(xdr->cx, fun, names);
    else if (ok)
