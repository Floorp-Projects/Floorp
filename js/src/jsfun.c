...
#if 0
        /*
         * FIXME: bug 375999 and bug 375808 cover similar ground.
         *
         * We should keep fun->object alive, as it is the object in which
         * hidden props for locals and formals were defined by the compiler.
         * If it dies before cloned function objects that haven't yet resolved
         * all of the needed hidden props, then such a cloned function object
         * may fail with an 'x is not defined' error when activated.
         *
         * It would suffice to copy the hidden props to an _heir apparent_
         * clone, if we could find one and point fun->object at it. But at
         * this point (finalization) we don't have any way to reach a clone
         * from obj.
         *
         * It turns out that Mozilla embeddings keep clone-parents alive via
         * the XUL prototype cache (which holds scripts, which hold functions
         * in atomPools).
         *
         * It might be better for clone-child and -parent to refer to the same
         * JSScope, even though there's no proto-slot linking child to parent.
         */
        if (fun->object)
