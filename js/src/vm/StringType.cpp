...
void JSLinearString::dumpRepresentationChars(js::GenericPrinter& out,
  ...
  const char* where =
      !sb || !sb->nursery().isInside(nonInlineCharsRaw()) ? "" : "(NURSERY) ";
  if (hasLatin1Chars()) {
...
bool AutoStableStringChars::init(JSContext* cx, JSString* s) {
  ...
  // If the chars are inline or nursery-allocated then we need to copy them
  // since they may be moved by a GC.
