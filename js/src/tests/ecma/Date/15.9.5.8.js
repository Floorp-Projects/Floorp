...
new TestCase( "(new Date(NaN)).getMonth()",
              NaN,
              (new Date(NaN)).getMonth() );

new TestCase( "Date.prototype.getMonth.length",
              0,
...
function addTestCase( t ) {
  ...
  for ( var m = 0; m < 12; m++ ) {
    ...
    new TestCase( "(new Date("+t+")).getMonth()",
                  MonthFromTime(LocalTime(t)),
                  (new Date(t)).getMonth() );

    new TestCase( "(new Date("+(t+1)+")).getMonth()",
    ...
    new TestCase( "(new Date("+(t-TZ_ADJUST)+")).getMonth()",
