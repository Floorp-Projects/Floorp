...
#  John Morrison <jrgm@netscape.com>
#  Mazie Lobo    <mazielobo@netscape.com>
#
#  Based on jsDriver.pl by rginda@netscape.com.
#  We have modified write_results() to output performance data,
#  and added send_results_to_server() from loader.pl by 
#  jrgm@netscape.com. 
#  
###############################################################
# NOTE: THE CALL TO send_results_to_server() IS COMMENTED OUT
# BELOW. IF YOU WANT TO POST TO |$server|, UNCOMMENT IT!!!
###############################################################

use strict;
use Getopt::Mixed "nextOption";

my $server = "http://jrgm.mcom.com/cgi-bin/jsperf/collect.cgi";
...
# required variables for timing modules
...
sub execute_tests {
    ...
    foreach $test (@test_list) {
        ...
        foreach $line (@output) {
			...
			if ($line =~ /([^\t]*)\t*TIME:[^\d]*(\d*)/) {
				...
				$time_reported++;
		          ...
		          # loop to print out different tests
		          if($test eq $test_prev)
				  {
				    $time_lines .= "<tr><td> &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</td>";
				  }
				  else
				  {
				     $time_lines .= "<tr><td>$test &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</td>";
				     $test_prev = $test;
				  }

				$time_lines .= "<td>$method &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</td>";
				$time_lines .= "<td>$methodtime</td></tr>\n";
			    $time_message .= "$test - $method \t $methodtime\n";
                ###############################################################
                # send_results_to_server($methodtime, $method, $test, $config);
                ###############################################################
            }
        ...
        if ($got_exit != $expected_exit) {
            ...
            &report_test ($test, $time_lines,
                           $bug_number, $time_message);
        } 
...
sub write_results {
	...
	print OUTPUT 
      ("<html><head>\n" .
       ...
       "<body bgcolor='white'>\n" .
       "<a name='tippy_top'></a>\n" .
       "<h1><center>Performance Results</center></h1><br>\n" .
       "<p class='results_summary'>\n" .
       "Test List: $list_name<br>\n" .
       "Skip List: $neglist_name<br>\n" .
       ($#test_list + 1) . " test(s) selected, " .
       "$time_reported timing result(s) reported, $failures_reported failure(s) occurred. <br>\n" .
    ...
    if ($time_reported > 0) {
        print OUTPUT
          ("[ <a href='#fail_detail'>Performance Details</a> | " .
           "<a href='#retest_list'>Test List</a> ]" .
           "<hr>\n" .
           "<a name='fail_detail'></a>\n" .
           "<font size=+2><b>Performance Details</b></font>&nbsp;&nbsp;(all times in milliseconds)<br>\n<dl><table border='1'>\n" .
           "<tr><th>Test</th><th>Method</th><th>Time</th></tr>".
		   $html .
           "</dl></table><br>\n[ <a href='#tippy_top'>Top of Page</a> | " .
           "<a href='#fail_detail'>Top of Tests</a> ]<br>\n" .
           "<hr>\n" .
           "<a name='retest_list'></a>\n" .
           "<font size=+2><b>List of Tests Executed</b></font>\n" .
		   "<pre>".
           ...
           "</pre>\n" .
           "[ <a href='#tippy_top'>Top of Page</a> | " .
           "<a href='#retest_list'>Top of Test List</a> ]<br>\n");
    } else {
        print OUTPUT 
          ("<h1>Nothing was tested!</h1>\n");
    }
    ...
    if ($opt_console_failures) {
        &status ("$tests_completed test(s) selected, $time_reported timing result(s) reported.");
		&status ("$failures_reported failure(s) reported.");
    }
}
...
sub report_test {
    my ($test, $message, $bug_number, $time_message) = @_;
    my $bug_line = "";
   ...
   $html .= "$message";
    ...
    @performance_tests[$#performance_tests + 1] = $test;
}


sub send_results_to_server {
    my ($methodtime, $method, $test, $config) = @_;

    my $tmpurl = $server;
    $tmpurl .= "?value=$methodtime&data=$method&testname=$method&tbox=$config";
    print STDERR "send_results_to_server(): \n";
    ...
    if ($@) {
        ...
        print STDERR "send_results_to_server() failed.\n";
    } else {
...
sub read_config_file{
      ...
      my $machineconfig;
        ...
        if (-f "config.txt")
		{
		    open( CONFIGFILE, "<config.txt" ) or die "Can't open config.txt : $!";
        }
		else
		{
            return "No machine specs!";
		}
       

        while( <CONFIGFILE> ) {
            $_ =~ s/#.*//;            # ignore comments by erasing them
            next if $_ =~ /^(\s)*$/;  # skip blank lines
            chomp;                    # remove trailing newline characters
            $machineconfig = $_;      # push the data line onto the array
            last;
        }
        close CONFIGFILE;
