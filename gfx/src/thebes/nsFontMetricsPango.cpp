                               ...
                               nsThebesRenderingContext *aContext)
{
    ...
    DrawStringSlowly(aString, NULL, aLength, aContext,
                     aX, aY, line, aSpacing);
                               ...
                               nsThebesRenderingContext *aContext)
{
    ...
    DrawStringSlowly(text, aString, aLength, aContext,
                     aX, aY, line, aSpacing);
...
static void DrawCairoGlyphs(cairo_t* aCairo, PangoFont* aFont, nscoord aX, nscoord aY,
                            PangoGlyphString* aGlyphs, float aP2T)
{
    PangoFcFont* fcfont = PANGO_FC_FONT(aFont);
    cairo_font_face_t* font = cairo_ft_font_face_create_for_pattern(fcfont->font_pattern);
    cairo_set_font_face(aCairo, font);
    ...
    cairo_set_font_size(aCairo, size);
    ...
    for (gint i = 0; i < aGlyphs->num_glyphs; ++i) {
        ...
        glyphs[i].x = (aX/aP2T) + (offset + info->geometry.x_offset)/PANGO_SCALE;
        glyphs[i].y = (aY/aP2T) + (info->geometry.y_offset)/PANGO_SCALE;
        offset += info->geometry.width;
    }
    cairo_show_glyphs(aCairo, glyphs, aGlyphs->num_glyphs);
                                     ...
                                     const nscoord *aSpacing)
{
     ...
     */
    nscoord *utf8spacing = nsnull;

    if (aSpacing) {
        utf8spacing = new nscoord[strlen(aText)];
        ...
        if (aOrigString) {
            ...
            for (PRUint32 curOffset=0; curOffset < aLength;
                 curOffset++, curChar = g_utf8_find_next_char(curChar, NULL)) {
                utf8spacing[curChar - aText] = (int)((float)aSpacing[curOffset] / p2t * PANGO_SCALE);
    ...
    for (GSList *tmpList = aLine->runs; tmpList && tmpList->data;
        ...
        for (gint i=0; i < layoutRun->glyphs->num_glyphs; i++) {
            ...
            gint thisOffset;
            if (aSpacing) {
                thisOffset = (gint)(utf8spacing[layoutRun->glyphs->log_clusters[i] + layoutRun->item->offset]);
                layoutRun->glyphs->glyphs[i].geometry.width = thisOffset;
            } else {
                thisOffset = layoutRun->glyphs->glyphs[i].geometry.width;
            }
        ...
        DrawCairoGlyphs(aContext->Thebes()->GetCairo(), layoutRun->item->analysis.font,
                        aX + nscoord(offset/PANGO_SCALE), aY,
                        layoutRun->glyphs, p2t);
