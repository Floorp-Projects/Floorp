...
/** ---------------------------------------------------
 */
nsPrintSettingsMac::nsPrintSettingsMac(const nsPrintSettings* aPS) :
    nsPrintSettings(aPS),
    mPrintRecord(nsnull)
{
  NS_INIT_REFCNT();
  
  nsPrintSettingsMac* aPSMac = (nsPrintSettingsMac*)aPS;
  mPrintRecord = aPSMac->mPrintRecord;
  if (mPrintRecord) {
    OSErr err = ::HandToHand((Handle*)&mPrintRecord);
    if (err) {
      NS_ERROR("Failed to copy print settings");
      mPrintRecord = nsnull;
    }
  }
}
...
/** ---------------------------------------------------
 */
nsresult nsPrintSettingsMac::CloneObj(nsIPrintSettings **_retval)
{
  nsPrintSettingsMac *clone = new nsPrintSettingsMac(this);
  if (!clone)
