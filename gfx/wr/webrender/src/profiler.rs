...
const fn float(name: &'static str, unit: &'static str, index: usize, expected: Expected<f64>) -> CounterDescriptor {
    CounterDescriptor { name, unit, show_as: ShowAs::Float, index, expected }
}

const fn int(name: &'static str, unit: &'static str, index: usize, expected: Expected<i64>) -> CounterDescriptor {
...
pub static PORFILE_COUNTERS: &'static[CounterDescriptor] = &[
...
impl Profiler {
    pub fn new() -> Self {
        ...
        let mut counters = Vec::with_capacity(PORFILE_COUNTERS.len());

        for (idx, descriptor) in PORFILE_COUNTERS.iter().enumerate() {
...
impl Expected<i64> {
    ...
    const fn into_float(self) -> Expected<f64> {
        Expected {
