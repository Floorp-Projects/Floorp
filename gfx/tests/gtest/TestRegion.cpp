...
TEST(Gfx, TiledRegionBoundaryConditions1) {
  ...
  EXPECT_TRUE(tiledRegion.Contains(nsIntRect(INT_MAX - 1, INT_MAX - 1, 1, 1)));
...
TEST(Gfx, TiledRegionBoundaryConditions2) {
  ...
  EXPECT_TRUE(tiledRegion.Contains(nsIntRect(INT_MIN, INT_MAX - 1, 1, 1)));
  EXPECT_FALSE(tiledRegion.Contains(nsIntRect(0, 0, 1, 1)));
}

TEST(Gfx, TiledRegionBigRects) {
  TiledIntRegion tiledRegion;
  // Giant region, forces simplification into bounds mode
  tiledRegion.Add(nsIntRegion(nsIntRect(INT_MIN, INT_MIN, INT_MAX, INT_MAX / 2)));
  EXPECT_TRUE(tiledRegion.Contains(nsIntRect(INT_MIN, INT_MIN, 1, 1)));
  EXPECT_TRUE(tiledRegion.Contains(nsIntRect(-2, INT_MIN / 2 - 2, 1, 1)));
  EXPECT_FALSE(tiledRegion.Contains(nsIntRect(-2, INT_MIN / 2 - 1, 1, 1)));
  EXPECT_FALSE(tiledRegion.Contains(nsIntRect(100, 100, 1, 1)));

  // Expand the bounds to cover everything
  tiledRegion.Add(nsIntRegion(nsIntRect(INT_MAX - 1, INT_MAX - 1, 1, 1)));
  EXPECT_TRUE(tiledRegion.Contains(nsIntRect(100, 100, 1, 1)));
  EXPECT_TRUE(tiledRegion.Contains(nsIntRect(INT_MAX - 1, INT_MAX - 1, 1, 1)));
  EXPECT_TRUE(tiledRegion.Contains(nsIntRect(INT_MAX, INT_MAX, 1, 1)));
}
...
TEST(Gfx, TiledRegionBoundaryOverflow) {
  ...
  EXPECT_TRUE(tiledRegion.Contains(nsIntRect(INT_MAX, INT_MAX, 100, 100)));
