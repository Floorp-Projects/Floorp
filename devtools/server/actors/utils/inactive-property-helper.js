...
class InactivePropertyHelper {
  ...
  /**
   * Check if the current node is an absolutely-positioned element to a grid container.
  ...
  isAbsPosGridElement() {
    // TODO: Determine if the element's containing block is generated by a grid and add
    // another check for that here. For now, we just check if the element is positioned
    // to avoid incorrect inactive CSS warnings.
    return (
