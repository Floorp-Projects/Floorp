...
function newChannelForURL(url, { policy, window, principal }) {
  ...
  // This is necessary because the absolute path of the test is C://path
  // which is parsed as a URL with scheme "c".
  const handler = Services.io.getProtocolHandler(uri.scheme);
  if (handler instanceof Ci.nsIExternalProtocolHandler) {
    if (!handler.externalAppExistsForScheme(uri.scheme)) {
      uri = Services.io.newURI("file://" + url);
    }
  }
