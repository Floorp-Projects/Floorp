...
function testGraph(host, graph) {
  ok(
    graph._container.classList.contains("flame-graph-widget-container"),
    "The correct graph container was created."
  );
  ok(
    graph._canvas.classList.contains("flame-graph-widget-canvas"),
    "The correct graph container was created."
  );
  ...
  is(
    graph.width,
    Math.round(bounds.width * window.devicePixelRatio),
    "The graph has the correct width."
  );
  is(
    graph.height,
    Math.round(bounds.height * window.devicePixelRatio),
