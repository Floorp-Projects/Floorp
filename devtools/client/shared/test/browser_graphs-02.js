...
function testDataAndRegions(graph) {
  ...
  for (let i = 0; i < TEST_REGIONS.length; i++) {
    ...
    is(
      original.start * graph.dataScaleX,
      normalized.start,
      "The region's start value was properly normalized."
    );
    is(
      original.end * graph.dataScaleX,
      normalized.end,
...
function testHighlights(graph) {
  ...
  ok(graph.hasMask(), "The graph should now have the highlights set.");

  graph.setMask([]);
  ok(graph.hasMask(), "The graph shouldn't have anything highlighted.");

  graph.setMask(null);
  ok(!graph.hasMask(), "The graph should have everything highlighted.");
}
