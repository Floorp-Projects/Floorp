...
TableWidget.prototype = {
   ...
   *        table's rows.
   */
  setColumns(columns, sortOn = this.sortedOn, hiddenColumns = [],
                        privateColumns = []) {
    for (let column of this.columns.values()) {
      column.destroy();
    }
...
Column.prototype = {
  ...
  onTableFiltered(event, itemsToHide) {
    this._updateItems();
    if (!this.cells) {
      return;
    }
