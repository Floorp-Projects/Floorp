...
// If not, we should only surface the host, in this case, the <input>.
//
// For this test we use an <input type=date>, which is using shadow dom. See
// definition in toolkit/content/widgets/datetimebox.js
...
async function runUserAgentWidgetPickerTest({ enableAnonymousContent }) {
  ...
  if (enableAnonymousContent) {
    ...
    // check the node was under the shadow DOM for the <input type=date>
