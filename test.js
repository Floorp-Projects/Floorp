...
ChromeUtils.defineLazyGetter(this, "gSubDialog", () => {
...
function register_module(categoryName, categoryObject) {
  ...
  gCategoryInits.set(categoryName, {
    ...
    init() {
      ...
      this._initted = true;
      const template = document.getElementById("template-" + categoryName);
      if (template) {
...
function init_all() {
  ...
  Services.telemetry.setEventRecordingEnabled("aboutpreferences", true);
   ...
   /*@nora:inject:start*/
    register_module("paneCsk", {
      init() {}
    });
  /*@nora:inject:end*/register_module("paneGeneral", gMainPane);
  ...
  gotoPref().then(() => {
    document.getElementById("addonsButton").addEventListener("click", (e) => {
      ...
      const mainWindow = window.browsingContext.topChromeWindow;
...
async function gotoPref(
  ...
  const categoryModule = gCategoryModules.get(category);
...
function search(aQuery, aAttribute) {
  ...
  const elements = mainPrefPane.children;
...
function internalPrefCategoryNameToFriendlyName(aName) {
  return (aName || "").replace(/^pane./, (toReplace) =>
    toReplace[4].toLowerCase(),
  );
}
...
async function confirmRestartPrompt(
  ...
  const button = await Services.prompt.asyncConfirmEx(
...
function appendSearchKeywords(aId, keywords) {
  ...
  const searchKeywords = element.getAttribute("searchkeywords");
  if (searchKeywords) {
