...
# a single .zip file to improve startup performance and memory footprint.
#
...
($debug >= 1) && print "\nZipiing .xpt files...\n";
($debug >= 2) && print "do_zip():\n";

# get list of directories on which to run zip
...
foreach $component (@xptdirs) {
	...
	if (( $os eq "MacOS" && -d "$PD$bindir$PD"."components") || ( -d "$bindir$PD"."components")) {
		...
		# merge .xpt files into zip if we found any in the dir
		if ( $#xptfiles >=1 ) {
			if ( $os eq "MacOS" ) {
				# XXXX what to do on Mac???
				$cmdline = "'$srcdir$PD"."xpt_link' $component".".xpt.new"." @xptfiles";
			} else {
				$cmdline = "zip $component"."-xpt.zip"." @xptfiles";
			}
			($debug >= 4) && print "$cmdline\n";
			system($cmdline);

			# remove old .xpt files in the component directory if xpt_link succeeded
			if	(( -f "$component"."-xpt.zip" ) ||
				(( -f "$PD$component"."-xpt.zip" ) && ( $os eq "MacOS" ))) {
...
($debug >= 1) && print "Zipping .xpt files completed.\n";
...
$0
	...
	.xpt files into single new .zip file for improved startup time and
