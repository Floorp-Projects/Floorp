...
static char * readFileToNewAllocatedZeroTerminatedBuffer(FILE * hFile)
{
  if(hFile == NULL)
    return NULL;
...
unsigned long XP_GetPrivateProfileString(char * szSection, char * szKey, char * szDefault, char * szString, unsigned long ulSize,  char * szFileName)
{
  ...
  if(szSection == NULL) // find all sections and return
  {
    ...
    for(;;)
    {
      ...
      {
        if(iBytes + iLength + 2 < (int)ulSize)
  ...
  //
  {
    ...
    int iLength = (iKeyLength < (int)ulSize - 1) ? iKeyLength : (int)ulSize - 1;
...
ReturnDefault:
  ...
  if((szDefault != NULL) && (szString != NULL))
    strcpy(szString, szDefault);
  return (unsigned long)strlen(szString);
}
                 ...
                 int iBytesToReplace) // number of bytes to replace
{
  ...
  int ipBufLength;
  if (pBuf) {
    ipBufLength = strlen(pBuf);
  } else {
    ipBufLength = 0;
  }
  
  int iNewLength = ipBufLength - iBytesToReplace + strlen(szString);
...
BOOL XP_WritePrivateProfileString(char * szSection, char * szKey, char * szString, char * szFileName)
{
  if((szSection == NULL) || (szKey == NULL) || (szString == NULL) || (szFileName == NULL))
    return FALSE;
