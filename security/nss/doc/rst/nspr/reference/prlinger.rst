...
``polarity``
   ...
   determine how long ``PR_Close`` waits before returning.
``linger``
   Time (in ``PRIntervalTime`` units) to linger before closing if any
...
By default, ``PR_Close`` returns immediately, but if there are any data
...
represented by a structure of type ``PRLinger``, makes it possible to
change this default as follows:

-  If ``polarity`` is set to ``PR_FALSE``, ``PR_Close`` returns
...
-  If ``polarity`` is set to ``PR_TRUE`` and ``linger`` is nonzero, the
   ...
   remains in the socket send buffer, ``PR_Close`` blocks until either
