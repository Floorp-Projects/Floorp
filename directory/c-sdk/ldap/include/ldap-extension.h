...
#define LDAP_CONTROL_EXT_WHOAMI         "1.3.6.1.4.1.4203.1.11.3"
...
/* where LDAP_CONTROL_PASSWD_POLICY control parse results */
...
#define LDAP_PP_WARNING_GRACE    0x02
        ...
        int          pp_error;         /* error information              v  */
#define LDAP_PP_ERROR_NONE      -1     /* no error                     (-1) */                         
#define LDAP_PP_ERROR_EXPIRED    0     /* password expired              (0) */
#define LDAP_PP_ERROR_LOCKED     1     /* account locked                (1) */
#define LDAP_PP_ERROR_MUSTCHANGE 2     /* paasword has to be changed    (2) */
#define LDAP_PP_ERROR_NOTMOD     3     /* modification not allowed      (3) */
#define LDAP_PP_ERROR_OLDPASSWD  4     /* must provide old password     (4) */
#define LDAP_PP_ERROR_NOQUALITY  5     /* insufficient password quality (5) */
#define LDAP_PP_ERROR_TOOSHORT   6     /* password too short            (6) */
#define LDAP_PP_ERROR_MINAGE     7     /* min age still in effect       (7) */
#define LDAP_PP_ERROR_INHISTORY  8     /* password in history           (8) */
} LDAPpwdpolicy;
...
LDAP_API(int) LDAP_CALL ldap_create_userstatus_control( LDAP *ld, const char ctl_iscritical,
														LDAPControl **ctrlp );
LDAP_API(int) LDAP_CALL ldap_parse_userstatus_control( LDAP *ld, LDAPControl **ctrlp,  
													   LDAPuserstatus *us );

LDAP_API(int) LDAP_CALL ldap_create_pwdpolicy_control( LDAP *ld, const char ctl_iscritical,
														  LDAPControl **ctrlp );
LDAP_API(int) LDAP_CALL ldap_parse_pwdpolicy_control( LDAP *ld, LDAPControl **ctrlp,  
														 LDAPpwdpolicy *pp );

LDAP_API(int) LDAP_CALL ldap_create_authzid_control( LDAP *ld, const char ctl_iscritical, 
													 LDAPControl **ctrlp );
LDAP_API(int) LDAP_CALL ldap_parse_authzid_control( LDAP *ld, LDAPControl **ctrlp, 
													char **authzid );

LDAP_API(int) LDAP_CALL ldap_whoami( LDAP *ld, LDAPControl **serverctrls, LDAPControl
									 **clientctrls, int *msgidp );
LDAP_API(int) LDAP_CALL ldap_whoami_s( LDAP *ld, LDAPControl **serverctrls, LDAPControl 
									   **clientctrls, char **authzid );
LDAP_API(int) LDAP_CALL ldap_parse_whoami( LDAP *ld, LDAPMessage *result, char **authzid );
 ...
 * ldap_create_proxiedauth_control() RFC 4370 control.
 */
...
LDAP_API(int) LDAP_CALL ldap_url_parse_no_defaults( const char *url, LDAPURLDesc **ludpp, int dn_required);
...
#define LDAP_X_OPT_SOCKETARG	(LDAP_OPT_PRIVATE_EXTENSION_BASE + 0x0F03)
        /* 0x4000 + 0x0F02 = 0x4F03 = 20227 - API extension */
