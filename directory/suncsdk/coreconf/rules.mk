...
ifeq ($(OS_TARGET),WIN16)
	...
	$(MKPROG) $(OBJS) -Fe$@ -link $(LDFLAGS) $(OS_LIBS) $(EXTRA_LIBS)
endif
else
	$(MKPROG) -o $@ $(CFLAGS) $(OBJS) $(LDFLAGS) $(OS_LIBS)
endif
...
$(OBJDIR)/$(PROG_PREFIX)%$(PROG_SUFFIX): $(OBJDIR)/$(PROG_PREFIX)%$(OBJ_SUFFIX)
	@$(MAKE_OBJDIR)
ifeq ($(OS_ARCH),WINNT)
	$(MKPROG) $(OBJDIR)/$(PROG_PREFIX)$*$(OBJ_SUFFIX) -Fe$@ -link $(LDFLAGS) $(OS_LIBS) $(EXTRA_LIBS)
else
	$(MKPROG) -o $@ $(OBJDIR)/$(PROG_PREFIX)$*$(OBJ_SUFFIX) $(LDFLAGS)
endif
...
#
ifneq ($(JNI_GEN),)
JNI_PACKAGE		:= $(subst /,.,$(PACKAGE))
JNI_CLASSES		:= $(patsubst %,$(JNI_PACKAGE).%,$(JNI_GEN))
JNI_JSRCS		:= $(patsubst %,%.java,$(JNI_GEN))
JNI_HEADERS		:= $(patsubst %,$(JNI_GEN_DIR)/%.h,$(JNI_GEN))

export_jni :: $(JNI_JSRCS)
	@if test ! -d $(JNI_GEN_DIR); then						\
		echo $(JAVAH) -jni -d $(JNI_GEN_DIR) $(JNI_CLASSES);			\
		$(JAVAH) -jni -d $(JNI_GEN_DIR) $(JNI_CLASSES);				\
	else										\
		echo "Checking for out of date header files" ;                          \
		cmd="perl $(CORE_DEPTH)/coreconf/jniregen.pl $(PERLARG)			\
				-d $(JAVA_DESTPATH) $(JNI_CLASSES)";			\
		echo $$cmd;								\
		list=`$$cmd`;								\
		if test "$${list}x" != "x"; then					\
...
ifneq ($(OS_ARCH),WINNT)
tests:: $(REGRESSION_SPEC) $(CORE_DEPTH)/$(MODULE)/scripts/regress.pl $(SOURCE_MD_DIR)/bin/reporter$(PROG_SUFFIX)
	cd $(PLATFORM); \
	perl5 -I ../$(CORE_DEPTH)/$(MODULE)/scripts \
              ../$(CORE_DEPTH)/$(MODULE)/scripts/regress.pl \
