 ...
 * this function MUST be called before ldapssl_enable_clienauth.
...
/* only used to support ldapssl_tls_start_s(); to be deprecated */
				 ...
				 LDAPControl **clientctrls)
{
	...
	oidptr	= NULL;
	dataptr	= NULL;
	if ( ( rc = ldap_extended_operation_s( ld, LDAP_EXOP_START_TLS, NULL, 
				serverctrls, clientctrls, &oidptr, &dataptr ) ) != LDAP_SUCCESS )
	{
		ber_bvfree( dataptr );
		ldap_memfree( oidptr );
		return( rc );
        ...
        char *keypasswd, char *certnickname )
{
    ...
    if ( keypasswd ) ssip->lssei_keypasswd = PL_strdup( keypasswd );
     ...
     */ 
    else ssip->lssei_using_pcks_fns = 1;
