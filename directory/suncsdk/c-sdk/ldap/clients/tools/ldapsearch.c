...
usage( void )
{
    ...
    fprintf( stderr, "    -F sep\tuse `sep' instead of `%s' between attribute names and values\n", LDAPTOOL_DEFSEP );
    fprintf( stderr, "          \tand print entries using old output format (attr=value)\n" );
    ...
    fprintf( stderr, "\t\t eg. \"nsroledn userPassword\"\n");
...
options_callback( int option, char *optarg )
{
	...
	break;
  case 'F':	/* print entries using old ldapsearch format and
  	         * and use optional field separator if provided. */
    if ( optarg && optarg[0] != '\0' ) sep = strdup( optarg );
    ldif = 0;
	break;
	case 'c':
