...
createRpm:
	...
	rm -rf $(RPM_DIR)
	mkdir -p $(RPM_DIR)/private/$(RPM_LIB)
	mkdir -p $(RPM_GENDIR)/BUILD
	mkdir -p $(RPM_GENDIR)/RPMS
	cp $(INSTDIR)/lib/libldap60.so $(RPM_DIR)/private/$(RPM_LIB)
	cp $(INSTDIR)/lib/libssldap60.so $(RPM_DIR)/private/$(RPM_LIB)
	cp $(INSTDIR)/lib/libprldap60.so $(RPM_DIR)/private/$(RPM_LIB)
	sed -e 's|%RPM_ROOTDIR%|$(RPM_ROOTDIR)|g' \
	    ...
	    -e 's|%RPM_PKGVERSION%|$(RPM_PKGVERSION)|g' \
            -e 's|%RPM_LIB%|$(RPM_LIB)|g' \
            -e 's|%RPM_ARCH%|$(RPM_ARCH)|g' \
	        ldapsdk.spec.template > $(RPM_GENDIR)/ldapsdk.spec
	rpmbuild -bb --target=$(RPM_ARCH) $(RPM_GENDIR)/ldapsdk.spec
...
#		rsh $(INTEGRATE_HOST) "$(INTEGRATE_CMD) -n -l ldap_sdk -r s9_lorion3 -R $(INSTDIR)/$(RPM_NAME)" ; \
#	else  \
...
patchRpm:
	...
	rm -rf $(RPM_PATCH_DIR)
	mkdir -p $(RPM_PATCH_DIR)/private/$(RPM_LIB)
	mkdir -p $(RPM_PATCH_GENDIR)/BUILD
	mkdir -p $(RPM_PATCH_GENDIR)/RPMS
	cp $(INSTDIR)/lib/libldap50.so $(RPM_PATCH_DIR)/private/$(RPM_LIB)
	cp $(INSTDIR)/lib/libssldap50.so $(RPM_PATCH_DIR)/private/$(RPM_LIB)
	cp $(INSTDIR)/lib/libprldap50.so $(RPM_PATCH_DIR)/private/$(RPM_LIB)
