                           ...
                           const nsReflowState& aReflowState)
{
  ...
  if ((nsnull != aState.lineLayout) &&
      (NS_OK == aKidFrame->QueryInterface(kIInlineReflowIID, (void**)&iir))) {
    rv = iir->InlineReflow(*aState.lineLayout, aDesiredSize, aReflowState);
  }
  else {
    aKidFrame->Reflow(aPresContext, aDesiredSize, aReflowState, rv);
...
// XXX inline reflow avoidance will be done by nsCSSLineLayout
#if XXX
...
{
  // XXX probably should create a line-layout object and use that
  // directly so that some semblance of proper behavior exists
  aStatus = Reflow2(aPresContext, nsnull, aDesiredSize, aReflowState);
  return NS_OK;/* XXX wrong; fix later */
}

NS_IMETHODIMP
nsInlineFrame::FindTextRuns(nsCSSLineLayout& aLineLayout)
{
  return NS_OK;
}

NS_IMETHODIMP
nsInlineFrame::InlineReflow(nsCSSLineLayout&    aLineLayout,
                            nsReflowMetrics&     aDesiredSize,
                            const nsReflowState& aReflowState)
{
  return Reflow2(aLineLayout.mPresContext, &aLineLayout, aDesiredSize,
                 aReflowState);
}

nsresult
nsInlineFrame::Reflow2(nsIPresContext*      aPresContext,
                       nsCSSLineLayout*    aLineLayout,
                       nsReflowMetrics&     aDesiredSize,
                       const nsReflowState& aReflowState)
{
  ...
  if (eReflowReason_Incremental == aReflowState.reason) {
    ...
    if (target == this) {
      ...
      case nsIReflowCommand::FrameAppended:
        ...
        rv = ReflowUnmappedChildren(aPresContext, state);
        break;
      ...
      // Reflow the child into the available space
      kidFrame->WillReflow(*aPresContext);
      rv = ReflowChild(state, kidFrame, aPresContext, kidSize, kidReflowState);
      ...
      if (!DidFitChild(aPresContext, state, kidFrame, kidSize)) {
        ...
        if (NS_FRAME_IS_COMPLETE(rv)) {
          ...
          rv = IncrementalReflowAfter(aPresContext, state, kidFrame, kidIndex);
