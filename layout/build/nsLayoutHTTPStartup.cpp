...
#include "nsLayoutHTTPStartup.h"
#include "nsIHTTPProtocolHandler.h"
#include "gbdate.h"

#define PRODUCT_NAME NS_LITERAL_STRING("Gecko").get()

NS_IMPL_ISUPPORTS1(nsLayoutHTTPStartup,nsIObserver)

nsresult
nsLayoutHTTPStartup::Observe( nsISupports *aSubject,
...
nsresult
nsLayoutHTTPStartup::RegisterHTTPStartup()
{
    nsresult rv;
    ...
    rv = catMan->AddCategoryEntry(NS_HTTP_STARTUP_CATEGORY,
                                  "Layout UserAgent Setter",
                                  NS_LAYOUTHTTPSTARTUP_CONTRACTID,
                                  PR_TRUE, PR_TRUE,
...
nsresult
nsLayoutHTTPStartup::UnregisterHTTPStartup()
{
    nsresult rv;
    nsCOMPtr<nsICategoryManager>
        catMan(do_GetService(NS_CATEGORYMANAGER_CONTRACTID, &rv));
