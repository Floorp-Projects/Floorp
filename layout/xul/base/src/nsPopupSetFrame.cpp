...
//
nsresult
NS_NewPopupSetFrame(nsIFrame** aNewFrame, PRUint32 aFlags)
{
...
NS_IMETHODIMP
nsPopupSetFrame::GetFrameForPoint(const nsPoint& aPoint, 
                              nsIFrame**     aFrame)
{
...
NS_IMETHODIMP 
nsPopupSetFrame::HandleEvent(nsIPresContext& aPresContext, 
                             nsGUIEvent*     aEvent,
...
void
nsPopupSetFrame::ToggleMenuState()
{  
  if (mMenuOpen) {
    OpenMenu(PR_FALSE);
  }
...
void
nsPopupSetFrame::SelectMenu(PRBool aActivateFlag)
{
  if (aActivateFlag) {
    // Highlight the menu.
...
PRBool nsPopupSetFrame::IsGenerated()
{
...
void nsPopupSetFrame::MarkAsGenerated()
{
  nsCOMPtr<nsIContent> child;
  GetMenuChildrenElement(getter_AddRefs(child));
...
void 
nsPopupSetFrame::OpenMenu(PRBool aActivateFlag) 
{
...
void
nsPopupSetFrame::GetMenuChildrenElement(nsIContent** aResult)
{
  PRInt32 count;
...
void 
nsPopupSetFrame::ShortcutNavigation(PRUint32 aLetter, PRBool& aHandledFlag)
{
  nsIFrame* frame = mPopupFrames.FirstChild();
  if (frame) {
...
void
nsPopupSetFrame::KeyboardNavigation(PRUint32 aDirection, PRBool& aHandledFlag)
{
  nsIFrame* frame = mPopupFrames.FirstChild();
  if (frame) {
    nsMenuPopupFrame* popup = (nsMenuPopupFrame*)frame;
...
void
nsPopupSetFrame::Escape(PRBool& aHandledFlag)
{
  nsIFrame* frame = mPopupFrames.FirstChild();
  if (frame) {
    nsMenuPopupFrame* popup = (nsMenuPopupFrame*)frame;
...
void
nsPopupSetFrame::SelectFirstItem()
{
  nsIFrame* frame = mPopupFrames.FirstChild();
  if (frame) {
    nsMenuPopupFrame* popup = (nsMenuPopupFrame*)frame;
...
void
nsPopupSetFrame::Notify(nsITimer* aTimer)
{
  // Our timer has fired.
...
NS_IMETHODIMP
nsPopupSetFrame::CreateAnonymousContent(nsISupportsArray& aAnonymousChildren)
{
...
void 
nsPopupSetFrame::SplitOnShortcut(nsString& aBeforeString, nsString& aAccessString, nsString& aAfterString)
{
  nsString value;
  nsString accessKey;
...
void 
nsPopupSetFrame::BuildAcceleratorText(nsString& aAccelString)
{
  nsString accelText;
