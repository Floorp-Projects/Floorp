             ...
             nsIFactory **aFactory)
{
    ...
    if (aClass.Equals(kResourceProtocolHandlerCID)) {
        rv = NS_NewGenericFactory(&fact, nsResourceProtocolHandler::Create);
    }
    else {
        rv = NS_ERROR_FAILURE;
    }
...
NSRegisterSelf(nsISupports* aServMgr , const char* aPath)
{
    ...
    rv = compMgr->RegisterComponent(kResourceProtocolHandlerCID,  
...
NSUnregisterSelf(nsISupports* aServMgr, const char* aPath)
{
    ...
    rv = compMgr->UnregisterComponent(kResourceProtocolHandlerCID, aPath);
    if (NS_FAILED(rv)) return rv;
