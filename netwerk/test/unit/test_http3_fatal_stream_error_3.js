...
// This is similar test to test_http3_fatal_stream_error.js, but
// the error is happening on a different neqo-necko code path.
...
Http3FailedListener.prototype = {
  ...
  onStopRequest: function testOnStopRequest(request, status) {
    if (Components.isSuccessCode(status)) {
      ...
      Assert.equal(status, Cr.NS_ERROR_NET_HTTP3_PROTOCOL_ERROR);
      this.finish(true);
    }
  },
};
...
add_task(async function test_setup() {
  ...
  httpsUri = "https://foo.example.com:" + h2Port + "/wrong_frame_before_header";
});
