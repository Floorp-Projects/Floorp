...
add_task(async function test_dontCoalesce() {
  ...
  override.addIPOverride("alt1.example.com", "::1");
...
add_task(async function test_doCoalesce() {
  ...
  override.addIPOverride("foo.example.com", "127.0.0.1");
  override.addIPOverride("foo.example.com", "::1");
  override.addIPOverride("alt2.example.com", "127.0.0.1");
  override.addIPOverride("alt2.example.com", "::1");
...
add_task(async function test_doCoalesceAggresive421() {
  let server = await createServer();
  ...
  override.addIPOverride("foo.example.com", "::1");
  override.addIPOverride("foo.example.com", "127.0.0.1");
  override.addIPOverride("alt1.example.com", "127.0.0.1");

  let { addr: addr1, status: status1 } = await openChan(
    `https://foo.example.com:${server.port()}/`
  );
  Assert.equal(status1, 200);
  Assert.equal(addr1, "::1");
  let { addr: addr2, status: status2 } = await openChan(
    `https://alt1.example.com:${server.port()}/`
  );

  Assert.equal(status2, 200);
  Assert.equal(addr2, "127.0.0.1");
  await server.stop();
});
