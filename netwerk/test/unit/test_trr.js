...
add_task(
  {
    // With the socket process enabled the telemetry will be recorded in the
    // socket process, so we're not able to access it here.
    skip_if: () =>
      Services.prefs.getBoolPref(
        "network.http.network_access_on_socket_process.enabled"
      ),
  },
  async function test_fallback_telemetry() {
    dns.clearCache(true);
    Services.prefs.setBoolPref(
