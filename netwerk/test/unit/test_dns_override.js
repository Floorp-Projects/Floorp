...
add_task(async function test_bad_IPs() {
  Assert.throws(
    () => override.addIPOverride("example.org", "example.org"),
    /NS_ERROR_UNEXPECTED/,
    "Should throw if input is not an IP address"
  );
  Assert.throws(
    () => override.addIPOverride("example.org", ""),
    /NS_ERROR_UNEXPECTED/,
    "Should throw if input is not an IP address"
  );
  Assert.throws(
  ...
  Assert.throws(
    () => override.addIPOverride("example.org", "1-2-3-4"),
    /NS_ERROR_UNEXPECTED/,
    "Should throw if input is not an IP address"
  );
});
...
add_task(async function test_ipv6() {
  let listener = new Listener();
  override.addIPOverride("example.org", "fe80::6a99:9b2b:6ccc:6e1b");
  dns.asyncResolve(
    "example.org",
    0,
    listener,
...
add_task(async function test_clearHostOverride() {
  override.addIPOverride("example.org", "2.2.2.2");
  override.addIPOverride("example.com", "2.2.2.2");
  override.clearHostOverride("example.org");
  let listener = new Listener();
  dns.asyncResolve(
    "example.org",
    0,
...
add_task(async function test_multiple_IPs() {
  override.addIPOverride("example.org", "2.2.2.2");
  override.addIPOverride("example.org", "1.1.1.1");
  override.addIPOverride("example.org", "::1");
  override.addIPOverride("example.org", "fe80::6a99:9b2b:6ccc:6e1b");
  let listener = new Listener();
  dns.asyncResolve(
    "example.org",
...
add_task(async function test_cname_flag() {
  ...
  override.addIPOverride("example.org", "2.2.2.2");
  override.setCnameOverride("example.org", "example.com");
  listener = new Listener();
  dns.asyncResolve(
    "example.org",
    Ci.nsIDNSService.RESOLVE_CANONICAL_NAME,
