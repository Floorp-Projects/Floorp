...
NS_IMETHODIMP
FuzzingStreamListener::OnStopRequest(nsIRequest* aRequest, nsresult aStatusCode) {
...
static int FuzzingRunNetworkHttp(const uint8_t *data, size_t size) {
...

  {
    ...
    loadFlags = nsIRequest::LOAD_BACKGROUND | nsIRequest::LOAD_BYPASS_CACHE |
                nsIRequest::INHIBIT_CACHING | nsIRequest::LOAD_FRESH_CONNECTION | nsIChannel::LOAD_INITIAL_DOCUMENT_URI;
    nsSecurityFlags secFlags;
    secFlags = nsILoadInfo::SEC_ALLOW_CROSS_ORIGIN_DATA_IS_NULL |
               nsILoadInfo::SEC_SANDBOXED;
    nsCOMPtr<nsIChannel> channel;
    rv = NS_NewChannel(getter_AddRefs(channel), url, nsContentUtils::GetSystemPrincipal(), secFlags,
...
MOZ_FUZZING_INTERFACE_RAW(FuzzingInitNetworkHttp, FuzzingRunNetworkHttp, NetworkHttp);
