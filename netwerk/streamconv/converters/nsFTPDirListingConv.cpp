...
nsFTPDirListingConv::AsyncConvertData(const char *aFromType, const char *aToType,
    ...
    MOZ_LOG(gFTPDirListConvLog, PR_LOG_DEBUG, 
...
nsFTPDirListingConv::OnDataAvailable(nsIRequest* request, nsISupports *ctxt,
    ...
    MOZ_LOG(gFTPDirListConvLog, PR_LOG_DEBUG, ("nsFTPDirListingConv::OnData(request = %x, ctxt = %x, inStr = %x, sourceOffset = %llu, count = %u)\n", request, ctxt, inStr, sourceOffset, count));
...
#ifndef DEBUG_dougt
    MOZ_LOG(gFTPDirListConvLog, PR_LOG_DEBUG, ("::OnData() received the following %d bytes...\n\n%s\n\n", streamLen, buffer.get()) );
#else
...
#ifndef DEBUG_dougt
    MOZ_LOG(gFTPDirListConvLog, PR_LOG_DEBUG, ("::OnData() sending the following %d bytes...\n\n%s\n\n", 
    ...
    if (line && *line) {
        mBuffer.Append(line);
        MOZ_LOG(gFTPDirListConvLog, PR_LOG_DEBUG, ("::OnData() buffering the following %d bytes...\n\n%s\n\n",
            strlen(line), line) );
    }
