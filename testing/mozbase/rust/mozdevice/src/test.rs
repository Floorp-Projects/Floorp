...
fn encode_message_with_valid_string() {
    ...
    assert_eq!(encode_message(&"a".repeat(15)).unwrap(), format!("000F{}", "a".repeat(15)));
    assert_eq!(encode_message(&"a".repeat(255)).unwrap(), format!("00FF{}", "a".repeat(255)));
    assert_eq!(
        encode_message(&"a".repeat(4095)).unwrap(), format!("0FFF{}", "a".repeat(4095))
    );
    assert_eq!(
        encode_message(&"a".repeat(65535)).unwrap(), format!("FFFF{}", "a".repeat(65535))
    );
}
...
fn run_device_test<F>(test: F) -> ()
    where F: FnOnce(&Device, &TempDir, &Path) -> () + panic::UnwindSafe
{
    ...
    let device = host.device_or_default::<String>(None)
    ...
    let result = panic::catch_unwind(|| {
        test(&device, &tmp_dir, &remote_path)
    });
...
fn host_device_or_default_valid_serial() {
    ...
    let device = host.device_or_default::<String>(Some(&expected_device.serial))
...
fn host_device_or_default_invalid_serial() {
    ...
    host.device_or_default::<String>(Some(&"foobar".to_owned())).expect_err("invalid serial");
}
...
fn host_device_or_default_no_serial() {
    ...
    let device = host.device_or_default::<String>(None).expect("connected device with serial");
    assert_eq!(device.serial, expected_device.serial);
}
...
fn device_shell_command() {
    run_device_test(|device: &Device, _: &TempDir, _: &Path| {
        assert_eq!(
            "Linux\n",
            device.execute_host_shell_command("uname").expect("to have shell output")
        );
    });
}
...
fn device_forward_port_hardcoded() {
    run_device_test(|device: &Device, _: &TempDir, _: &Path| {
        assert_eq!(3035, device.forward_port(3035, 3036).expect("forwarded local port"));
...
fn device_kill_forward_port_no_forwarded_port() {
    run_device_test(|device: &Device, _: &TempDir, _: &Path| {
        device.kill_forward_port(3038).expect_err("adb error: listener 'tcp:3038' ");
    });
}
...
fn device_kill_forward_port_twice() {
    run_device_test(|device: &Device, _: &TempDir, _: &Path| {
        let local_port = device.forward_port(3039, 3040).expect("forwarded local port");
        assert_eq!(local_port, 3039);
        // TODO: check with forward --list
        device.kill_forward_port(local_port).expect("to remove forwarded port");
        device.kill_forward_port(local_port).expect_err("adb error: listener 'tcp:3039' ");
    });
}
...
fn device_kill_forward_all_ports_no_forwarded_port() {
    run_device_test(|device: &Device, _: &TempDir, _: &Path| {
        device.kill_forward_all_ports().expect("to not fail for no forwarded ports");
    });
}
...
fn device_kill_forward_all_ports_twice() {
    run_device_test(|device: &Device, _: &TempDir, _: &Path| {
        let local_port1 = device.forward_port(3039, 3040).expect("forwarded local port");
        assert_eq!(local_port1, 3039);
        let local_port2 = device.forward_port(3041, 3042).expect("forwarded local port");
        assert_eq!(local_port2, 3041);
        // TODO: check with forward --list
        device.kill_forward_all_ports().expect("to remove all forwarded ports");
        device.kill_forward_all_ports().expect("to not fail for no forwarded ports");
    });
}
...
fn device_kill_reverse_port_no_reverse_port() {
    run_device_test(|device: &Device, _: &TempDir, _: &Path| {
        device.kill_reverse_port(4038).expect_err("listener 'tcp:4038' not found");
    });
}
...
fn device_kill_reverse_port_twice() {
    run_device_test(|device: &Device, _: &TempDir, _: &Path| {
        let remote_port = device.reverse_port(4039, 4040).expect("reversed local port");
        assert_eq!(remote_port, 4039);
        // TODO: check with reverse --list
        device.kill_reverse_port(remote_port).expect("to remove reverse port");
        device.kill_reverse_port(remote_port).expect_err("listener 'tcp:4039' not found");
    });
}
...
fn device_kill_reverse_all_ports_no_reversed_port() {
    run_device_test(|device: &Device, _: &TempDir, _: &Path| {
        device.kill_reverse_all_ports().expect("to not fail for no reversed ports");
    });
}
...
fn device_kill_reverse_all_ports_twice() {
    run_device_test(|device: &Device, _: &TempDir, _: &Path| {
        let local_port1 = device.forward_port(4039, 4040).expect("forwarded local port");
        assert_eq!(local_port1, 4039);
        let local_port2 = device.forward_port(4041, 4042).expect("forwarded local port");
        assert_eq!(local_port2, 4041);
        // TODO: check with reverse --list
        device.kill_reverse_all_ports().expect("to remove all reversed ports");
        device.kill_reverse_all_ports().expect("to not fail for no reversed ports");
    });
}
...
fn device_push() {
    run_device_test(|device: &Device, _: &TempDir, remote_root_path: &Path| {
        ...
        device.push(&mut io::BufReader::new(content.as_bytes()), &remote_path, 0o777)
...
#[test]
fn device_push_dir() {
    run_device_test(|device: &Device, tmp_dir: &TempDir, remote_root_path: &Path| {
        let content = "test";
        ...
        device.push_dir(tmp_dir.path(), &remote_root_path, 0o777)
            .expect("to push_dir");
